...
class InternetFetcher:
    ...
    def _check_cache(self, url: str, etag: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    def fetch(self, url: str, scope: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self._is_allowed_domain(url):
            parsed = urlparse(url)
            domain = parsed.netloc.lower()
            allowed_list = sorted(ALLOWED_DOMAINS) + [f"*.{pattern.lstrip('.')}" for pattern in ALLOWED_DOMAIN_PATTERNS]
            return {
                'success': False,
