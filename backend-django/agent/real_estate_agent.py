...
try:
    mcp_server_path = os.path.join(
        os.path.dirname(__file__), "..", "api_mcp", "server.py"
    )
    ...
    warnings.warn(
        f"MCP server not available: {e}. Agent will have limited functionality."
    )
...
def _patch_langchain_tool_parser():
    ...
    try:
        ...
        _original_parse_openai = (
            openai_tools_module.parse_ai_message_to_openai_tool_action
        )
            ...
            try:
                ...
                if "'NoneType' object has no attribute 'get'" in str(e):
                    ...
                    if hasattr(message, "tool_calls") and message.tool_calls:
                        for tool_call in message.tool_calls:
                            ...
                            if hasattr(tool_call, "args") and tool_call.args is None:
                                ...
                                and tool_call.get("args") is None
                            ):
                                tool_call["args"] = {}
                    if (
                    ...
                    try:
                        ...
                        return AgentFinish(
                            return_values={
                                "output": "אירעה שגיאה בעיבוד הבקשה. אנא נסה לנסח מחדש את השאלה."
                            },
        ...
        # Apply patches
        tools_module.parse_ai_message_to_tool_action = (
            _safe_parse_ai_message_to_tool_action
        )
        ...
        if hasattr(openai_tools_module, "parse_ai_message_to_tool_action"):
            ...
            openai_tools_module.parse_ai_message_to_tool_action = (
                _safe_parse_ai_message_to_tool_action
            )
...
def _safe_int_env(var_name: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning(
            "Invalid integer for %s: %s. Using default %s.", var_name, value, default
        )
        return default
...
def _get_tokenizer():
    ...
    try:
        ...
        logger.error(
            "tiktoken is required for token budgeting but is not installed: %s", exc
        )
        raise
...
def enforce_token_budget(messages, max_tokens: int, reserve_for_output: int = 1024):
    ...
    for msg in reversed(messages):  # newest → oldest
        ...
        if running + c <= target:
            ...
            trimmed.append(
                {"role": "system", "content": "[history summarized due to budget]"}
            )
            break
    trimmed.reverse()
    return trimmed
...
class ToolCallTracker(BaseCallbackHandler):
    ...
    def on_tool_start(
        ...
        logger.info(
            "🔧 Tool started: %s with input: %s", tool_name, str(input_str)[:100]
        )
...
class StreamingCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        logger.debug(
            "Received LLM token: %s (total length: %d)",
            token[:50],
            len(self.complete_response),
        )
    ...
    def on_tool_start(
        ...
        tool_name = serialized.get("name", "unknown_tool")
        logger.info(
            "🔧 Tool started (streaming): %s with input: %s",
            tool_name,
            str(input_str)[:100],
        )
        tool_hebrew = translate_tool_name(tool_name)
    ...
    def mark_finished(self):
        ...
        try:
            ...
            logger.warning(
                "Queue full when trying to add finished event, attempting to make space"
            )
            ...
            try:
                ...
                logger.error(
                    "Failed to add finished event even after making space: %s", e
                )
...
class RealEstateAgent:
    ...
    def __init__(
        ...
        self.history_summary_max_chars = _safe_int_env(
            "AGENT_HISTORY_SUMMARY_MAX_CHARS", 600
        )
        ...
        if self.api_token:
            ...
            logger.warning(
                "No API token provided to agent - MCP tools may not work properly"
            )
    ...
    def _create_llm(
        self, provider: str, temperature: float, user_api_key: Optional[str] = None
    ):
        """Create LLM instance based on provider."""

        # Helper to get API key from user key first, then env or Django settings
        def get_api_key(
            env_var_name: str, settings_attr: str = None, expected_provider: str = None
            ...
            if user_api_key and expected_provider:
                ...
                if (
                    expected_provider in provider_key_map
                    and env_var_name == provider_key_map[expected_provider]
                ):
        ...
        if provider == "openai":
            api_key, key_source = get_api_key(
                "OPENAI_API_KEY", "OPENAI_API_KEY", expected_provider=provider
            )
            if not api_key or not api_key.strip():
                logger.error(
                    "OPENAI_API_KEY is missing or empty (source: %s)", key_source
                )
                raise ValueError("OPENAI_API_KEY is required and must not be empty")
            if len(api_key.strip()) < 10:
                logger.warning(
                    "OPENAI_API_KEY appears to be too short (likely invalid) - length: %d, source: %s",
                    len(api_key.strip()),
                    key_source,
                )
            ...
            logger.info(
                "Using OpenAI LLM - Model: %s, Key source: %s, Key preview: %s",
            ...
            api_key1, source1 = get_api_key(
                "GEMINI_API_KEY", "GEMINI_API_KEY", expected_provider=provider
            )
            api_key2, source2 = get_api_key(
                "GOOGLE_API_KEY", "GOOGLE_API_KEY", expected_provider=provider
            )
            api_key = api_key1 or api_key2
            key_source = source1 if api_key1 else source2
            if not api_key or not api_key.strip():
                logger.error(
                    "GEMINI_API_KEY/GOOGLE_API_KEY is missing or empty (checked: GEMINI=%s, GOOGLE=%s)",
                    source1,
                    source2,
                )
                raise ValueError(
                    "GEMINI_API_KEY or GOOGLE_API_KEY is required and must not be empty"
                )
            key_preview = (
                f"{api_key[:10]}...{api_key[-4:]}"
                if len(api_key) > 14
            ...
            logger.info(
                "Using Gemini LLM - Model: %s, Key source: %s, Key preview: %s",
            ...
            api_key, key_source = get_api_key(
                "GROQ_API_KEY", "GROQ_API_KEY", expected_provider=provider
            )
            if not api_key or not api_key.strip():
                logger.error(
                    "GROQ_API_KEY is missing or empty (source: %s)", key_source
                )
            ...
            logger.info(
                "Using Groq LLM - Model: %s, Key source: %s, Key preview: %s",
                groq_model,
                key_source,
                key_preview,
            )
            ...
            if not BEDROCK_AVAILABLE:
                raise ValueError(
                    "langchain-aws is required for Bedrock support. Install it with: pip install langchain-aws"
                )
            ...
            aws_region = os.getenv("BEDROCK_AWS_REGION") or getattr(
                django_settings, "BEDROCK_AWS_REGION", None
            )
            if not aws_region:
            ...
            aws_access_key_id = (
                ...
                or getattr(django_settings, "BEDROCK_AWS_ACCESS_KEY_ID", None)
                or getattr(django_settings, "AWS_ACCESS_KEY_ID", None)
            )
            aws_secret_access_key = (
                os.getenv("BEDROCK_AWS_SECRET_ACCESS_KEY")
                or os.getenv("AWS_SECRET_ACCESS_KEY")
                or getattr(django_settings, "BEDROCK_AWS_SECRET_ACCESS_KEY", None)
                or getattr(django_settings, "AWS_SECRET_ACCESS_KEY", None)
            )

            # Get model ID
            bedrock_model = os.getenv("BEDROCK_MODEL_ID") or getattr(
                django_settings,
                "BEDROCK_MODEL_ID",
            ...
            logger.info(
                "Using AWS Bedrock LLM - Model: %s, Region: %s",
                bedrock_model,
                aws_region,
            )
    ...
    def _create_tools(self) -> List[BaseTool]:
        ...
        def _find_best_city_match(
            city: str, available_cities: List[str]
        ) -> Optional[str]:
            ...
            for available_city in available_cities:
                ...
                if (
                    city_normalized in available_normalized
                    or available_normalized in city_normalized
                ):
                    return available_city
            ...
            if RealEstateAgent._available_cities_cache is None:
                try:
                    ...
                    if isinstance(result, dict) and result.get("success"):
                        ...
                        RealEstateAgent._available_cities_cache = filters.get(
                            "cities", []
                        )
                    else:
        ...
        async def _enrich_asset_list_item(asset: Dict[str, Any]) -> Dict[str, Any]:
            ...
            asset_neighborhood = (
                asset.get("neighborhood") or asset.get("neighborhood_name") or ""
            )

            # If we don't have address components, try to get full asset data
            if not asset_city and not asset_street:
                try:
                    full_asset_result = await get_asset(
                        ctx, asset_id, include_documents=False
                    )
                    if isinstance(full_asset_result, dict) and full_asset_result.get(
                        "success"
                    ):
                        ...
                        asset_neighborhood = (
                            full_asset.get("neighborhood") or asset_neighborhood
                        )
                except Exception as e:
                    logger.debug(
                        f"Could not fetch full asset {asset_id} for address: {e}",
                        exc_info=True,
                    )
            ...
            try:
                listings_result = await get_asset_data(
                    ctx, asset_id=asset_id, kind="listings", limit=1, compact=False
                )
                enriched_data = {
                    "address": address,
                    "id": asset_id,
        ...
        async def _enrich_list_output(
            ...
            if not enrichment_func:
                ...
                result_text = "\n".join(
                    f"{i + 1}. {item}" for i, item in enumerate(formatted_items)
                )
                if len(data) > limit:
            ...
            for item in data[:limit]:
                try:
                    ...
                    logger.debug(
                        f"Error enriching item in {tool_name}: {e}", exc_info=True
                    )
                ...
                if city:
                    ...
                    if available_cities:
                        ...
                        if matched_city and matched_city != city:
                            ...
                            logger.info(
                                f"Auto-corrected city name: '{city}' -> '{corrected_city}'"
                            )
        ...
        async def get_asset_filters_tool_func() -> str:
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("success"):
                    ...
                    cities_str = ", ".join(cities[:20]) + (
                        "..." if len(cities) > 20 else ""
                    )
                    types_str = ", ".join(filters.get("types", [])[:10]) + (
                        "..." if len(filters.get("types", [])) > 10 else ""
                    )
                    neighborhoods_str = ", ".join(
                        filters.get("neighborhoods", [])[:10]
                    ) + ("..." if len(filters.get("neighborhoods", [])) > 10 else "")
                    return (
                        f"מסננים זמינים:\n"
                        + f"ערים: {cities_str}\n"
        ...
        class GetAssetDataInput(BaseModel):
            ...
            kind: str = Field(
                description="Type of data to retrieve - one of: transactions, permits, plans, appraisal, listings, documents"
            )
            fields: Optional[List[str]] = Field(
                default=None, description="Optional list of field names to return"
            )
            limit: Optional[int] = Field(
                default=5, description="Maximum number of items to return (default: 5)"
            )
...
{breakdown_text}"""
        ...
        def _format_mortgage_analysis(data: Dict[str, Any]) -> str:
            ...
            if recommendation:
                ...
                recommended_payment = recommendation.get(
                    "recommended_monthly_payment", 0
                )
                ...
                if recommended_payment > 0:
                    ...
                    max_loan_from_payment = recommendation.get(
                        "max_loan_from_payment", 0
                    )
                    if max_loan_from_payment == 0 and approved_loan > 0:
                        # Estimated payment based on LTV loan
                        lines.append(
                            f"  • תשלום חודשי משוער: ₪{recommended_payment:,.0f} (מבוסס על LTV 70%)"
                        )
                    else:
                        # Calculated from actual income data
                        lines.append(
                            f"  • תשלום חודשי מומלץ: ₪{recommended_payment:,.0f}"
                        )
                else:
                    lines.append(
                        "  • תשלום חודשי מומלץ: ₪0 (לא ניתן לחשב ללא נתוני הכנסות)"
                    )

                # Show cash gap information
                if cash_gap > 0:
                    lines.append(
                        f"  ⚠️  פער הון עצמי: ₪{cash_gap:,.0f} (חסר לקניית הנכס)"
                    )
        ...
        async def analyze_mortgage_tool_func(
            ...
            try:
                result = await analyze_mortgage(
                    ctx, property_price, savings_total, annual_rate_pct, term_years
                )
                if isinstance(result, dict) and result.get("success"):
                    data = result.get("data", {})
        ...
        async def list_leads_tool_func(
            ...
            try:
                ...
                result = await list_leads(
                    ctx, status=status, fields=fields, limit=limit, compact=True
                )
                if isinstance(result, dict) and result.get("success"):
                    data = result.get("data", [])
        ...
        if self.internet_enabled:
            ...
            async def fetch_web_page_tool_func(
                url: str, scope: Optional[str] = None
            ) -> str:
    ...
    def _create_agent_executor(self) -> AgentExecutor:
        ...
        language_sentence = "ענה בעברית למשתמשים בעברית ועבור לשפה אחרת רק אם התבקשת במפורש. הצג נתונים בצורה ברורה בעברית."
        ...
        def handle_parsing_error(error: Exception) -> str:
            ...
            if (
                "'NoneType' object has no attribute 'get'" in error_msg
    ...
    def _prepare_history_for_llm(
        ...
        tool_budget_content = (
            tool_budget_message.get("content") if tool_budget_message else None
        )
        ...
        for index, msg in enumerate(trimmed_messages):
            ...
            if (
                tool_budget_content
                and role == "system"
                and content == tool_budget_content
            ):
                continue
        ...
        try:
            if track_tool_calls:
                ...
                if formatted_tool_output and not response_text:
                    logger.info(
                        "Using formatted tool output as final response (no LLM response)"
                    )
                ...
                # If we have both, check if LLM response already includes the tool output
                    ...
                    has_tool_outputs = any(
                        ...
                        for keyword in ["₪", "מחיר:", "**", "סטטוס:", "•"]
                    )
                    if not has_tool_outputs:
                        logger.info(
                            "LLM response doesn't include tool outputs, but tool output is formatted - using tool output"
                        )
                        response_text = formatted_tool_output
                ...
                if not formatted_tool_output:
                    ...
                    for tool_call in tool_calls:
                        if tool_call.get("output"):
                            tool_hebrew = tool_call.get(
                                "tool_hebrew", tool_call.get("tool", "")
                            )
                    ...
                    if tool_outputs_section:
                        ...
                        if not response_text:
                            ...
                            has_tool_outputs = any(
                                keyword in response_text
                                for keyword in ["id:", "נמצאו", "property", "נכס", "₪"]
                            )
    ...
    async def chat_stream(self, message: str, chat_history: Optional[List] = None):
            ...
            try:
                ...
                async def stream_with_timeout():
                    ...
                    async for chunk in self.agent_executor.astream(
                        ...
                        logger.debug(
                            "Agent step %d received: %s", chunk_count, str(chunk)[:200]
                        )
                        ...
                        if isinstance(chunk, dict):
                            ...
                            for key in [
                                ...
                                if key in chunk:
                                    ...
                                    if isinstance(value, str) and value:
                                        final_output = value
                                        logger.info(
                                            "Found final output in chunk[%s]: %s",
                                            key,
                                            value[:100],
                                        )
                                        break
                                        ...
                                        for msg in value:
                                            if hasattr(msg, "content") and msg.content:
                                                ...
                                                logger.info(
                                                    "Found final output in chunk[%s] message: %s",
                                                    key,
                                                    final_output[:100],
                                                )
                                                break
                            ...
                            if (
                                final_output
                                and final_output not in callback.complete_response
                            ):
                                logger.info(
                                    "Final output not in callback, adding it directly"
                                )
                                callback.complete_response = final_output
                                # Send as a single chunk event
                                try:
                                    callback.chunk_queue.put_nowait(
                                        {"type": "chunk", "content": final_output}
                                    )
                                except asyncio.QueueFull:
                    ...
                    logger.info(
                        "Agent astream finished. Total chunks: %d, Last chunk keys: %s",
                        chunk_count,
                        last_chunk_keys,
                    )
                    logger.info(
                        "Last chunk content: %s",
                        str(last_chunk)[:500] if last_chunk else "None",
                    )
                        ...
                        if tool_name in list_tool_names:
                            ...
                            if output and (
                                ...
                                if not callback.complete_response:
                                    logger.info(
                                        "Setting complete_response from tool output after astream finished"
                                    )
                                    callback.complete_response = output
                                # Mark finished immediately since we have tool output
                                logger.info(
                                    "Have tool output after astream finished, marking finished immediately"
                                )
                    ...
                    if (
                        not callback.complete_response
                        and not final_output
                        and not has_tool_output
                    ):
                        logger.warning(
                            "Agent executor finished but no LLM response was generated"
                        )
                        # Try to extract from last chunk one more time
                        if last_chunk and isinstance(last_chunk, dict):
                            logger.info(
                                "Attempting to extract output from last chunk: %s",
                                list(last_chunk.keys()),
                            )

                await asyncio.wait_for(
                    stream_with_timeout(), timeout=300.0
                )  # 5 minute timeout
                logger.info(
                    "Agent execution completed successfully. Final output from chunks: %s",
                    final_output,
                )
                logger.info(
                    "Callback complete_response length: %d",
                    len(callback.complete_response),
                )
                logger.info(
                    "Callback complete_response content: %s",
                    callback.complete_response[:500]
                ...
                logger.info(
                    "After wait, callback complete_response length: %d",
                    len(callback.complete_response),
                )
                ...
                if final_output:
                    ...
                    if len(final_output) > len(callback.complete_response):
                        logger.info(
                            "Final output from chunks is longer, updating callback response"
                        )
                        callback.complete_response = final_output
                    elif final_output not in callback.complete_response:
                        logger.info(
                            "Final output not in callback response, appending it"
                        )
                        callback.complete_response += final_output
                    ...
                    for tool_call in tool_calls:
                        ...
                        if tool_name in list_tool_names:
                            ...
                            if output and (
                                ...
                                logger.info(
                                    "No LLM response but have formatted tool output - using it"
                                )
                                callback.complete_response = output
                                # Mark finished immediately when we have tool output, don't wait for LLM
                                logger.info(
                                    "Have tool output, marking finished immediately"
                                )
                                callback.mark_finished()
                                break

                logger.info(
                    "Final callback response length after processing: %d",
                    len(callback.complete_response),
                )
                logger.info(
                    "Final callback response preview: %s",
                    callback.complete_response[:200]
                    if callback.complete_response
                    else "Empty",
                )
                ...
                if not callback._finished:
                    ...
                    callback.mark_finished()
                logger.info(
                    "Callback marked as finished, _finished=%s", callback._finished
                )
            except asyncio.TimeoutError:
                ...
                try:
                    ...
                    try:
                        callback.chunk_queue.put_nowait(
                            {
                                "type": "error",
                                "error": "אירעה שגיאה בעת ביצוע הסוכן. אנא נסה שוב.",
                            }
                        )
                    except Exception:
                        pass
        ...
        try:
            ...
            max_consecutive_timeouts = (
                20  # Stop after 2 seconds of no events (20 * 0.1s)
            )
            ...
            while True:
                            ...
                            try:
                                ...
                                if event and event.get("type") == "error":
                                    ...
                                    if event.get("type") in [
                                        ...
                                        event["tool_hebrew"] = translate_tool_name(
                                            event.get("tool", "")
                                        )
                                    yield event
                            except Exception:
                ...
                if elapsed > max_wait_time:
                    logger.warning(
                        "Agent chat stream timeout after %s seconds", max_wait_time
                    )
                    yield {
                        "type": "error",
                ...
                if event:
                    ...
                    if event.get("type") == "astream_finished":
                        logger.info(
                            "Agent executor astream finished, will break after final processing"
                        )
                        final_timeouts = 0
                        for _ in range(10):  # 10 * 0.1s = 1 second
                            ...
                            if final_event:
                                ...
                                if final_event.get("type") in [
                                    "tool_call_start",
                                    "tool_call_end",
                                    "tool_call_error",
                                ]:
                                    final_event["tool_hebrew"] = translate_tool_name(
                                        final_event.get("tool", "")
                                    )
                                yield final_event
                            else:
                                final_timeouts += 1
                                ...
                                if final_timeouts >= 5:
                                    logger.info(
                                        "No more events after astream finished, breaking"
                                    )
                                    agent_finished = True
                                    break
                    ...
                    if agent_task.done():
                        ...
                        logger.warning(
                            "No events received for %s seconds, but agent still running. Checking task status...",
                        ...
                        if agent_task.done():
                            agent_finished = True
                            logger.info(
                                "Agent task finished during timeout check, breaking"
                            )
                            break
                        # Check if we have a complete response - if so, break even if task isn't done
                        if callback.complete_response:
                            logger.info(
                                "Have complete response but agent still processing, breaking early"
                            )
                            agent_finished = True
                            break
                        ...
                        if elapsed > 10.0:
                            logger.warning(
                                "Agent task still running after 10 seconds with no events, forcing break"
                            )
                            agent_finished = True
                            break
                        # If still running but less than 10 seconds, reset counter and continue
                        logger.warning(
                            "Agent task still running, continuing to wait..."
                        )
                        consecutive_timeouts = (
                            0  # Reset to allow more time, but log more frequently
                        )
            ...
            if not error_occurred and agent_finished:
                ...
                logger.info(
                    "Agent finished. Response length: %d, Tool calls: %d",
                    len(final_response) if final_response else 0,
                    len(tool_calls),
                )
                logger.info(
                    "Final response content (first 200 chars): %s",
                ...
                if not has_formatted_results:
                    ...
                    if tool_outputs_section:
                        ...
                        if not final_response:
                            ...
                            if not has_tool_outputs:
                                logger.info(
                                    "Appending tool outputs to streamed response"
                                )
                                final_response = f"{final_response}\n\n---\n\n**תוצאות החיפוש:**\n{tool_outputs_text}"
                            else:
                                logger.info(
                                    "Streamed response already includes tool outputs"
                                )
                else:
                    logger.info(
                        "Skipping tool outputs in stream - response already contains formatted results"
                    )
                ...
                logger.info(
                    "Sending complete event with response length: %d",
    ...
    def _extract_error_message(self, exception: Exception) -> str:
        ...
        if "model" in error_lower and (
            "decommissioned" in error_lower or "no longer supported" in error_lower
        ):
