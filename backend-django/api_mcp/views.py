...
def _get_mcp_tools_list():
    ...
    try:
        ...
        if hasattr(mcp, "_tool_manager"):
            ...
            if hasattr(tool_manager, "_tools"):
                for tool_name, tool_obj in tool_manager._tools.items():
                    try:
                        ...
                        if hasattr(tool_obj, "parameters") and isinstance(
                            tool_obj.parameters, dict
                        ):
            ...
            if tools:
                logger.info(
                    f"Successfully retrieved {len(tools)} tools from FastMCP tool manager"
                )
                return tools

        # Fallback: log warning
        logger.warning(
            "Could not access FastMCP tool manager. Tools may not be available via Django view."
        )
        return []
    ...
    if request.method == "GET":
        try:
            ...
            oauth_well_known_url = (
                f"{base_url}/mcp/.well-known/oauth-authorization-server"
            )
            ...
            icon_url = (
                "https://www.nadlaner.com/favicon.svg"  # Use marketing site favicon
            )
                ...
                return JsonResponse(
                    {
                        ...
                        "result": {
                            ...
                            "capabilities": {"tools": {}},
                            "serverInfo": {"name": "RealEstateAPI", "version": "1.0.0"},
                        },
                    }
                )
