    ...
    if hasattr(mcp, "http_app"):
        ...
        if mcp_lifespan:
            ...
            if callable(mcp_lifespan):
                logger.info(
                    "FastMCP lifespan is callable, will initialize task group on startup"
                )
            else:
                logger.warning(
                    "FastMCP lifespan found but is not callable. Type: %s",
                    type(mcp_lifespan),
                )
        else:
            logger.error(
                "FastMCP lifespan not found. mcp_app type: %s, dir: %s",
                type(mcp_app),
                [attr for attr in dir(mcp_app) if not attr.startswith("_")],
            )
        ...
        if mcp_lifespan:
            ...
            logger.error(
                "WARNING: No lifespan to pass to Starlette - FastMCP may not work!"
            )
