...
class BedrockAdapter(LLMClient):
    ...
    def __init__(self) -> None:
        ...
        aws_access_key_id = getattr(settings, "AWS_ACCESS_KEY_ID", None) or getattr(
    ...
    def _prepare_messages(self, messages: List[ChatMessage]) -> List[Dict[str, str]]:
        ...
        for msg in messages:
            ...
            bedrock_messages.append({"role": role, "content": msg.content})
    ...
    async def generate_text(
        ...
        try:
            ...
            response_body = await asyncio.to_thread(
                self._invoke_model, bedrock_messages, opts.temperature, False
            )
        except AttributeError:
            # Fallback for Python < 3.9
            loop = asyncio.get_event_loop()
            response_body = await loop.run_in_executor(
                None, self._invoke_model, bedrock_messages, opts.temperature, False
            )
    ...
    async def generate_json(
        ...
        try:
            # Python 3.9+ has asyncio.to_thread
            response_body = await asyncio.to_thread(
                self._invoke_model, bedrock_messages, opts.temperature, True
            )
        except AttributeError:
            # Fallback for Python < 3.9
            loop = asyncio.get_event_loop()
            response_body = await loop.run_in_executor(
                None, self._invoke_model, bedrock_messages, opts.temperature, True
            )
