...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--email", type=str, help="Email address to test with", default=None
        )
        parser.add_argument(
            "--phone", type=str, help="Phone number to test with", default=None
        )
        parser.add_argument(
            "--user-id",
            type=int,
    ...
    def test_whatsapp_alert(self, phone=None):
        ...
        try:
            alert.send(test_message)
            self.stdout.write(
                self.style.SUCCESS("  ✅ WhatsApp alert sent successfully")
            )
        except Exception as e:
    ...
    def test_user_notifier(self, user_id=None):
        ...
        try:
            ...
            if notifier:
                self.stdout.write(
                    self.style.SUCCESS("  ✅ User notifier created successfully")
                )
                self.stdout.write(f"     - Email enabled: {user.notify_email}")
                self.stdout.write(f"     - WhatsApp enabled: {user.notify_whatsapp}")
                self.stdout.write(
                    f"     - Number of alert channels: {len(notifier.alerts)}"
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        "  ⚠️  No notifier created (user may not have notification channels enabled)"
                    )
                )
