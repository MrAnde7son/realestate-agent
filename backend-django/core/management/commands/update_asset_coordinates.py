            ...
            if 'x' in govmap_data and 'y' in govmap_data:
                ...
                if 100_000 <= x_itm <= 400_000 and 500_000 <= y_itm <= 1_000_000:
                    ...
                    if self.is_valid_israel_coords(lat_wgs84, lon_wgs84):
                        ...
                        logger.warning(f"GovMap coordinates {x_itm}, {y_itm} converted to invalid WGS84: {lat_wgs84}, {lon_wgs84}")
                else:
                    logger.warning(f"GovMap coordinates {x_itm}, {y_itm} don't look like ITM (expected x: 100k-400k, y: 500k-1000k)")
        ...
        if asset.city and ('תל אביב' in asset.city or 'tel aviv' in asset.city.lower()):
            try:
                if asset.street and asset.number:
                    x_itm, y_itm = gis_collector._geocode(asset.street, asset.number, asset.city or '')
                    lon_wgs84, lat_wgs84 = itm_to_wgs84(x_itm, y_itm)
        ...
        for i, asset in enumerate(assets, 1):
            try:
                ...
                if not force and self.is_valid_israel_coords(asset.lat, asset.lon):
                    skipped += 1
                    if i % 100 == 0:
                        self.stdout.write(f'Progress: {i}/{total} (updated: {updated}, skipped: {skipped}, failed: {failed})')
                    continue
                ...
                # Geocode
                self.stdout.write(f'Processing asset {asset.id} ({i}/{total}): {asset.street} {asset.number}, {asset.city}')
                lat, lon, source = self.geocode_asset(asset, govmap_collector, gis_collector)

                if lat is None or lon is None:
                    self.stdout.write(self.style.ERROR(
                        f'Asset {asset.id}: Failed to geocode'
                    ))
                    failed += 1
                    continue
