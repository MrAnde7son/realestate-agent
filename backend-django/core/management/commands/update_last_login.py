...
class Command(BaseCommand):
    help = (
        "Update last_login field for users who have logged in but have null last_login"
    )
    ...
    def handle(self, *args, **options):
        ...
        users_to_update = User.objects.filter(
            last_login__isnull=True, updated_at__gte=thirty_days_ago
        )
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "No users found with null last_login that need updating."
                )
            )
            return
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated last_login for {updated_count} users."
                )
            )
