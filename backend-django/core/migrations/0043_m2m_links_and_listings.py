...
def backfill_docs(apps, schema_editor):
    ...
    for document in Document.objects.exclude(asset=None).only('id', 'asset_id'):
        AssetDocument.objects.get_or_create(document_id=document.id, asset_id=document.asset_id)
...
def backfill_transactions(apps, schema_editor):
    ...
    for transaction in transaction_model.objects.exclude(asset=None).only('id', 'asset_id'):
        AssetTransaction.objects.get_or_create(transaction_id=transaction.id, asset_id=transaction.asset_id)
...
def dedupe_yad2_to_listing(apps, schema_editor):
    ...
    qs = SourceRecord.objects.filter(source='yad2').exclude(external_id__isnull=True).only(
        'id', 'external_id', 'title', 'url', 'raw', 'asset_id'
    )
    for source_record in qs.iterator():
        ...
        if source_record.asset_id:
            AssetListing.objects.get_or_create(listing_id=listing.id, asset_id=source_record.asset_id)
