...
class PlanService:
    ...
    def assign_free_plan_to_user(user: User) -> UserPlan:
        ...
        user_plan = UserPlan.objects.create(
            user=user, plan_type=free_plan, is_active=True
        )
    ...
    def update_asset_usage(user: User, delta: int = 1):
        ...
        try:
            ...
            if user_plan:
                ...
                logger.info(
                    f"Updated asset usage for user {user.email}: {user_plan.assets_used}"
                )
        except Exception as e:
            ...
            return {
                ...
                "limits": {
                    "assets": {
                        ...
                        "used": assets_used,
                        "remaining": plan_type.asset_limit - assets_used
                        if plan_type.asset_limit != -1
                        else -1,
                    },
                    "reports": {
                        "limit": plan_type.report_limit,
                        "used": user_plan.reports_used,
                        "remaining": plan_type.report_limit - user_plan.reports_used
                        if plan_type.report_limit != -1
                        else -1,
                    },
                    "alerts": {
                        "limit": plan_type.alert_limit,
                        "used": user_plan.alerts_used,
                        "remaining": plan_type.alert_limit - user_plan.alerts_used
                        if plan_type.alert_limit != -1
                        else -1,
                    },
                },
                "features": {
    ...
    def upgrade_user_plan(user: User, new_plan_name: str) -> UserPlan:
        ...
        try:
            ...
            # Create new plan
            user_plan = UserPlan.objects.create(
                user=user, plan_type=new_plan_type, is_active=True
            )
            ...
            logger.error(
                f"Error upgrading user {user.email} to plan {new_plan_name}: {e}"
            )
