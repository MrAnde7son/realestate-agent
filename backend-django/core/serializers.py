...
class AssetSerializer(MetaSerializerMixin):
    ...
    legalRestrictions = serializers.CharField(
        source="legal_restrictions", read_only=True
    )
    urbanRenewalPotential = serializers.CharField(
        source="urban_renewal_potential", read_only=True
    )
    ...
    buildingCoveragePct = serializers.FloatField(
        source="building_coverage_pct", read_only=True
    )
    ...
    def get_price_per_sqm(self, obj):
        ...
        if area_val in (None, ""):
            area_val = self._get_primary_value(
                obj, "size", "area", "square_meters", "squareMeters"
            )
        try:
    ...
    def _listing_sort_key(self, listing):
        ...
        if hasattr(self, "_current_asset_address"):
            ...
            normalized_addr = (
                getattr(self.parent.instance, "normalized_address", "") or ""
            )
        ...
        if asset_address and listing_address:
            ...
            if asset_address == listing_address:
                return (
                    ...
                    0,
                    0,
                )  # Priority: exact match, full timestamp, 0 for tie-breaker
    ...
    class Meta:
        ...
        fields = [
            ...
            "parcel_accuracy",
            "block_area",
            "block_registered_area",
            "block_total_parcels",
            "block_status",
            "block_last_update",
            "total_permits",
            "permit_request_num",
            "permit_permission_num",
            "permit_building_num",
            "permit_housing_units",
            "permit_commercial_area",
            "permit_residential_area",
            "permit_residential_units",
            "permit_public_area",
            "permit_parking_area",
            "permit_parking_units",
            "permit_small_apartments",
            "permit_unified_housing_area",
            "permit_unified_housing_units",
            "permit_accessible_apartments",
            "permit_public_built_area",
            "permit_total_area",
            "permit_mavat_plan_num",
            "permit_parking_rooms_calculated",
            "permit_full_utilization",
            "permit_subject_type",
            "permit_process",
            "permit_rights_notification",
            "permit_repartition",
            "permit_urban_renewal",
            "permit_open_request_date",
            "permit_construction_start_date",
        ]
...
class PermitSerializer(MetaSerializerMixin):
    ...
    def get_assets(self, instance):
        assets_qs = (
            instance.all_assets()
            if hasattr(instance, "all_assets")
            else instance.assets.all()
        )
    ...
    def get_request_num(self, instance):
        ...
        return (
            raw.get("request_num") or raw.get("meta", {}).get("request_num")
    ...
    def get_is_tama_38(self, instance):
        ...
        return (
            raw.get("sw_tama_38")
            or raw.get("sw_tama_38_chadash")
            or raw.get("sw_tama_38_tosefet")
            or meta.get("sw_tama_38")
            or meta.get("sw_tama_38_chadash")
            or meta.get("sw_tama_38_tosefet")
            or False
        )
    ...
    def get_has_construction_allowances(self, instance):
        ...
        meta = raw.get("meta", {}) if isinstance(raw.get("meta"), dict) else {}
        return bool(
            raw.get("hakala_tosefet_achuz_shetach")
            or raw.get("hakala_yd_hagdala_achuz")
            or raw.get("hakala_yd_mevukash")
            or raw.get("hakala_yd_mutar")
            or meta.get("hakala_tosefet_achuz_shetach")
            or meta.get("hakala_yd_hagdala_achuz")
            or meta.get("hakala_yd_mevukash")
            or meta.get("hakala_yd_mutar")
        )
    ...
    def get_display_title(self, instance):
        ...
        return (
            ...
            or f"Permit {instance.permit_number}"
        )
    ...
    def _sync_assets(self, instance, asset_ids):
        ...
        if remove_ids:
            AssetPermit.objects.filter(
                permit=instance, asset_id__in=remove_ids
            ).delete()
...
class AlertRuleSerializer(serializers.ModelSerializer):
    ...
    def validate_params(self, value):
        ...
        if trigger_type == "PRICE_DROP":
            ...
            if not isinstance(pct, (int, float)) or pct < 0 or pct > 100:
                raise serializers.ValidationError(
                    "Price drop percentage must be between 0 and 100"
                )
            ...
            if (
                ...
                raise serializers.ValidationError(
                    "Market trend delta percentage must be between 0 and 100"
                )
            if not isinstance(window_days, int) or window_days < 7 or window_days > 180:
                raise serializers.ValidationError(
                    "Window days must be between 7 and 180"
                )
            if (
                not isinstance(radius_km, (int, float))
                or radius_km < 0.1
                or radius_km > 5
            ):
            ...
            if (
                ...
                raise serializers.ValidationError(
                    "Radius must be between 50 and 2000 meters"
                )
    ...
    def validate(self, data):
        ...
        if data.get("scope") == "asset" and not data.get("asset"):
            raise serializers.ValidationError(
                "Asset must be provided when scope is 'asset'"
            )
        return data
...
class AlertEventSerializer(serializers.ModelSerializer):
    ...
    def get_asset_address(self, obj):
        ...
        if obj.asset:
            return (
                obj.asset.normalized_address
                or f"{obj.asset.street} {obj.asset.number}, {obj.asset.city}"
            )
        return None
...
class AssetContributionSerializer(serializers.ModelSerializer):
    ...
    def get_contribution_type_display(self, obj):
        ...
        return translations.get(
            obj.contribution_type, obj.get_contribution_type_display()
        )
...
class AssetWithAttributionSerializer(AssetSerializer):
    ...
    last_updated_by_email = serializers.CharField(
...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    def _collect_assets(self, instance):
        ...
        if (
            ...
            and "assets" in instance._prefetched_objects_cache
        ):
    ...
    def _sync_assets(self, instance, asset_ids):
        ...
        if remove_ids:
            AssetDocument.objects.filter(
                document=instance, asset_id__in=remove_ids
            ).delete()
...
class DocumentUploadSerializer(serializers.Serializer):
    ...
    document_type = serializers.ChoiceField(
        choices=Document.DOCUMENT_TYPE_CHOICES, default="other"
    )
    ...
    def _sync_assets(self, instance, asset_ids):
        ...
        for asset_id in add_ids:
            AssetTransaction.objects.get_or_create(
                transaction=instance, asset_id=asset_id
            )
        if remove_ids:
            AssetTransaction.objects.filter(
                transaction=instance, asset_id__in=remove_ids
            ).delete()
    ...
    def _sync_assets(self, instance, asset_ids):
        ...
        if remove_ids:
            AssetListing.objects.filter(
                listing=instance, asset_id__in=remove_ids
            ).delete()
