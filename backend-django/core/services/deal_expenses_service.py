...
class DealExpensesService:
    ...
    def _calc_regular(self, value: float, is_first: bool) -> float:
        ...
        brackets = self.SINGLE_HOME_BRACKETS if is_first else self.ADDITIONAL_HOME_BRACKETS
    ...
    def _calc_disabled(self, value: float, is_first: bool) -> float:
        ...
        remaining = self._calc_regular(value - self.DISABLED_CAP, is_first) if value > self.DISABLED_CAP else 0.0
    ...
    def calculate_purchase_tax(
        ...
        for buyer in buyers:
            share_pct = buyer.get("sharePct", 0)
            portion_price = (price * share_pct) / 100
            is_first = buyer.get("isFirstHome", False) or buyer.get("isReplacementHome", False)
            
            regular = self._calc_regular(portion_price, is_first)
            oleh = self._calc_oleh(portion_price) if buyer.get("oleh", False) else float('inf')
            disabled = self._calc_disabled(portion_price, is_first) if buyer.get("disabled", False) else float('inf')
            bereaved = self._calc_disabled(portion_price, is_first) if buyer.get("bereavedFamily", False) else float('inf')
    ...
    def calculate_deal_expenses(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            purchase_tax_result = self.calculate_purchase_tax(price, buyers, property_type)
            
            # Calculate service costs
            service_costs_result = self.calculate_service_costs(price, services, vat_rate)
