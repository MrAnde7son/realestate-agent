    ...
    def post(self, request, asset_id):
        ...
        try:
            ...
            if not data.get("title"):
                inferred_title = (
                    getattr(uploaded_file, "name", None) if uploaded_file else None
                )
...
class AssetRightsView(APIView):
    ...
    permission_classes = [
        AllowAny
    ]  # Changed from [IsAuthenticated] to allow public access
    ...
    def get(self, request, asset_id):
        try:
            ...
            documents = (
                ...
                .filter((Q(asset=asset) | Q(assets=asset)), document_type="tabu")
                .distinct()
            ...
            logger.info(
                f"Asset {asset_id}: GIS data from meta: {list(gis_data.keys()) if gis_data else 'empty'}"
            )

            # Also get the comprehensive GIS collector data (stored directly in meta)
            gis_collector_data = (
                asset.meta.get("gis_collector_data", {}) if asset.meta else {}
            )
            if gis_collector_data:
                ...
                logger.info(
                    f"Asset {asset_id}: Found GIS collector data with {len(gis_collector_data)} keys: {list(gis_collector_data.keys())}"
                )
            else:
                logger.info(
                    f"Asset {asset_id}: No GIS collector data found in metadata. Available meta keys: {list(asset.meta.keys()) if asset.meta else 'no meta'}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Error calculating building rights for asset {asset.id}: {e}"
                )
                import traceback
            ...
            if privilege_data_list:
                ...
                if isinstance(privilege_data_list, list):
                    ...
                    for idx, privilege_data in enumerate(privilege_data_list):
                        if privilege_data:
                            detailed_rights = self._process_privilege_page_data(
                                privilege_data
                            )
    ...
    def _parse_ownership_from_tabu(self, tabu_data):
        ...
        ownership = {"owners": [], "total_ownership_percentage": 0, "parcel_info": {}}
        ...
        for row in tabu_data:
            ...
            # Collect area information with clearer naming
                ...
                ownership["parcel_info"]["subparcel_area"] = (
                    value  # Clearer than 'net_area'
                )
            elif "שטח בנוי" in field or "built area" in field:
                ownership["parcel_info"]["built_area"] = value
            elif (
                "שטח" in field
                and "כולל" not in field
                and "נטו" not in field
                and "בנוי" not in field
            ):
                # Generic area field
        ...
        for i, owner_name in enumerate(owner_names):
                ...
                try:
                    ...
                    if "/" in share_value:
                        ...
                        if fraction_match:
                            ...
                            logger.debug(
                                f"Parsed fraction {share_value} as {ownership_percentage}%"
                            )
                    # Handle "בשלמות" (full ownership) = 100%
                        ...
                        if percentage_match:
                            ...
                            logger.debug(
                                f"Parsed percentage {share_value} as {ownership_percentage}%"
                            )
            ...
            if ownership_percentage > 0:
                ownership["owners"].append(
                    {"name": owner_name, "percentage": ownership_percentage}
                )
                logger.debug(f"Added owner: {owner_name} with {ownership_percentage}%")

        # Calculate total ownership percentage
        ownership["total_ownership_percentage"] = sum(
            owner["percentage"] for owner in ownership["owners"]
        )
    ...
    def _calculate_building_rights(self, asset):
        ...
        parcel_area = (
            meta.get("parcelArea", {}).get("value")
            if isinstance(meta.get("parcelArea"), dict)
            else meta.get("parcelArea")
        )
        parcel_registered_area = (
            meta.get("parcelRegisteredArea", {}).get("value")
            if isinstance(meta.get("parcelRegisteredArea"), dict)
            else meta.get("parcelRegisteredArea")
        )

        calculated_rights["parcel_info"] = {
            "parcel_area_sqm": parcel_area,
            "parcel_registered_area_sqm": parcel_registered_area,
            "area_source": meta.get("parcelArea", {}).get("source", "GIS")
            if isinstance(meta.get("parcelArea"), dict)
            else "GIS",
        }
        ...
        if isinstance(privilege_data_list, dict):
            ...
            privilege_data = next(
                (
                    ...
                    and (item.get("rights") or item.get("rights_details"))
                ),
                None,
            )
        ...
        if privilege_data:
            ...
            if isinstance(privilege_data, dict) and (
                "rights" in privilege_data or "rights_details" in privilege_data
            ):
                rights = privilege_data.get("rights") or privilege_data.get(
                    "rights_details", {}
                )
                logger.debug(
                    f"Rights type: {type(rights)}, keys: {list(rights.keys()) if isinstance(rights, dict) else 'Not a dict'}"
                )
                ...
                def _is_area_entry(entry: Dict[str, Any]) -> bool:
                    ...
                    if entry_type in ("building_area", "auxiliary_building_area"):
                        return True
                    raw_text_lower = (
                        raw_text.lower() if isinstance(raw_text, str) else ""
                    )
                    non_area_markers = [
                        "תיסופיט",
                        "הינש",
                    ...
                    if any(marker in raw_text for marker in non_area_markers) or any(
                        marker in raw_text_lower for marker in non_area_markers
                    ):
                        return False
                    area_keywords = [
                        'מ"ר',
                    ...
                    return any(kw in raw_text for kw in area_keywords) or any(
                        kw in unit for kw in unit_area_markers
                    )
                ...
                if not total_building_privilege:
                    ...
                    if isinstance(main_rights, dict):
                        try:
                            total_building_privilege = float(
                                main_rights.get("value") or 0
                            )
                        except (TypeError, ValueError):
                ...
                for fp in floor_percentages:
                    ...
                    if floor_type and percentage > 0:
                        ...
                        is_standard_floor = floor_type in [
                            "טיפוסית",
                            "ראשונה",
                        ] or "טיפוסית" in fp.get("raw_text", "")

                        floor_area = (
                            (total_building_privilege * percentage / 100)
                            if total_building_privilege > 0
                            else 0
                        )
                ...
                for fp in floor_percentages:
                    ...
                    if floor_type and percentage > 0:
                        ...
                        if (
                            is_relative
                            and base_floor_type
                            and base_floor_type in absolute_floors
                        ):
                ...
                if isinstance(parking_req, str):
                    ...
                    for pr in parking_req:
                        if isinstance(pr, dict):
                            ...
                            parking_requirements.append(
                                {"area_sqm": None, "raw_text": pr, "value": pr}
                            )
                ...
                calculated_rights["building_privileges"] = {
                    ...
                    "auxiliary_building_area_sqm": float(auxiliary_area)
                    if auxiliary_area
                    else 0,
                ...
                calculated_rights["privilege_page_details"] = {
                    ...
                    "basic_info": privilege_data.get(
                        "basic_info", privilege_data.get("basic", {})
                    ),
                    "alerts": privilege_data.get("alerts", []),
                    "plans_count": len(
                        privilege_data.get("plans", {})
                        .get("in_force", {})
                        .get("local", [])
                    )
                    if privilege_data.get("plans")
                    else 0,
                }
                    ...
                    current_building_details.append(
                        {
                            ...
                            "residential_area_sqm": float(residential_area)
                            if residential_area
                            else 0,
                            "commercial_area_sqm": float(commercial_area)
                            if commercial_area
                            else 0,
                        }
                    )
        ...
        total_privilege = calculated_rights.get("building_privileges", {}).get(
            "total_building_privilege_sqm", 0
        )
        remaining_area = total_privilege - total_current_area
        remaining_percentage = (
            (remaining_area / total_privilege * 100) if total_privilege > 0 else 0
        )
        ...
        calculated_rights["current_usage"]["usage_percentage"] = (
            (total_current_area / total_privilege * 100) if total_privilege > 0 else 0
        )
        ...
        if asset.price_per_sqm and remaining_area > 0:
            ...
            estimated_rights_value = round(
                (asset.price_per_sqm * remaining_area * 0.7) / 1000
            )
        ...
        calculated_rights["summary"] = {
            ...
            "utilization_percentage": calculated_rights["current_usage"][
                "usage_percentage"
            ],
    ...
    def _process_privilege_page_data(self, privilege_data):
        ...
        for alert in alerts:
            detailed_rights["alerts"].append(
                {"type": "alert", "text": alert, "source": "privilege_page"}
            )
            detailed_rights["rights_details"].append(
                {
                    "type": "alert",
        ...
        if rights.get("building_coverage_percentage"):
            detailed_rights["percentages"]["building_coverage"] = rights[
        ...
        for note in notes:
            if isinstance(note, dict):
                ...
                detailed_rights["notes"].append(
                    {"text": str(note), "type": "general", "source": "privilege_page"}
                )
                detailed_rights["rights_details"].append(
                    {
                        "type": "note",
    ...
    def _process_gis_data_sources(self, gis_data, rights_data, query):
        ...
        if query:
            rights_data["gis_rights"] = [
                row
                for row in rights_data["gis_rights"]
    ...
    def get(self, request, asset_id, document_id):
        ...
        try:
            ...
            if not (
                ...
                or any(
                    getattr(asset, "created_by", None) == request.user
...
class DocumentDownloadView(APIView):
    ...
    def get(self, request, asset_id, document_id):
        ...
        try:
            ...
            logger.info(
                f"Document {document_id} is_downloadable: {document.is_downloadable}"
            )
            logger.info(
                f"Document {document_id} default_storage.exists: {default_storage.exists(document.file_path)}"
            )
            ...
            try:
                ...
                response = StreamingHttpResponse(
                    file_generator(), content_type=document.mime_type
                )
    ...
    try:
        ...
        for doc_data in documents:
            ...
            if Document.objects.filter(
                (Q(asset=asset) | Q(assets=asset)), external_id=doc_data.get("id")
            ).exists():
                continue
...
def _update_asset_from_tabu(asset, tabu_rows):
    ...
    try:
        ...
        if ownership_data.get("owners"):
            ...
            primary_owner = max(
                ownership_data["owners"], key=lambda x: x.get("percentage", 0)
            )
        ...
        if ownership_data.get("owners"):
            ...
            logger.info(
                f"Updated asset {asset.id} ownership from Tabu: {primary_owner['name']} ({primary_owner['percentage']}%)"
            )
        else:
            logger.info(
                f"Updated asset {asset.id} from Tabu data (no ownership info found)"
            )
        ...
        for row in tabu_rows:
            ...
            if "שטח כולל" in field or "total area" in field:
                ...
                if not asset.area:
                    ...
                    logger.debug(
                        f"Updated area field from Tabu subparcel: {area_value}"
                    )
                ...
                if not asset.total_area:
                    ...
                    update_fields.add("total_area")
                    logger.debug(
                        f"Updated total_area from generic area field: {area_value}"
                    )

        if update_fields:
            logger.info(
                f"Updated asset {asset.id} area fields from Tabu: {list(update_fields)}"
            )
