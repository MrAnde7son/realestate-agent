...
from .analytics import (
    ...
    track_contact_updated,
    track_contact_deleted,
    track_lead_created,
    track_lead_updated,
    track_lead_deleted,
    track_lead_status_changed,
    track_lead_note_added,
    track_lead_report_sent,
)
...
class Contact(models.Model):
    ...
    tags = models.JSONField(
        default=list, blank=True, help_text="Tags like ['משקיע', 'VIP']"
    )
...
class Lead(models.Model):
    ...
    asset = models.ForeignKey(
        "core.Asset", on_delete=models.CASCADE, related_name="leads"
    )
        ...
        if not is_new:
            ...
            if changed_fields:
                ...
                if "status" in changed_fields:
                    track_lead_status_changed(
                        self, self.contact.owner_id, old_lead.status, self.status
                    )

        # Keep total tasks count in sync with the component counts
        self.total_tasks_count = (self.completed_tasks_count or 0) + (
            self.pending_tasks_count or 0
        )
    ...
    def add_note(self, note_text: str, user_id: int = None):
        ...
        notes = self.notes or []
        notes.append({"ts": self.last_activity_at.isoformat(), "text": note_text})
        self.notes = notes
