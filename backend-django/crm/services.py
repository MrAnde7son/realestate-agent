...
צוות נדל"נר
        ...
        try:
            ...
            track_asset_change_notified(
                asset, asset.created_by.id, leads.count(), change_summary
            )
...
def send_report_to_contact(lead_id: int, report_payload: dict):
    ...
    try:
        ...
        pdf_content = build_branded_pdf(
            payload=report_payload, owner_id=contact.owner_id
        )
...
def get_asset_leads_summary(asset_id: int) -> dict:
    ...
    summary = {"total_leads": leads.count(), "by_status": {}, "contacts": []}
...
def send_email(
    subject: str, message: str, recipient_list: list, from_email: str = None
):
    """
...
def build_branded_pdf(payload: dict, owner_id: int) -> BytesIO:
    ...
    # Report type
    p.drawString(
        100, y_position, f"סוג דוח: {payload.get('report_type', 'property_analysis')}"
    )
    y_position -= 20
