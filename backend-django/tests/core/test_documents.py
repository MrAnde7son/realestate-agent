...
class DocumentStorageTest(TestCase):
    ...
    def test_validate_file(self):
        ...
        invalid_file = SimpleUploadedFile(
            "test.exe", b"file content", content_type="application/x-executable"
        )
...
class DocumentAPITest(APITestCase):
    ...
    def test_get_document_detail(self):
        ...
        url = reverse(
            "document_detail",
            kwargs={"asset_id": self.asset.id, "document_id": document.id},
        )
        response = self.client.get(url)
    ...
    def test_update_document(self):
        ...
        data = {"title": "Updated Document", "description": "Updated description"}
...
class DocumentTableAccessTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="table-tester", email="table@example.com", password="testpass123"
        )
        self.asset = Asset.objects.create(
            scope_type="address",
            city="Tel Aviv",
...
class DocumentSerializerTest(TestCase):
    ...
    def test_document_serializer_updates_asset_ids(self):
        ...
        serializer = DocumentSerializer(
            document, data={"asset_ids": [other_asset.id]}, partial=True
        )
        ...
        document.refresh_from_db()
        self.assertEqual(
            list(document.assets.values_list("id", flat=True)), [other_asset.id]
        )
        self.assertEqual(
            sorted(a.id for a in document.all_assets()),
            sorted([self.asset.id, other_asset.id]),
        )
