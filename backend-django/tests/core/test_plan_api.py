...
class TestPlanAPIEndpoints:
    ...
    def test_user_plan_info_authenticated(self):
        ...
        UserPlan.objects.create(
            user=self.user, plan_type=self.basic_plan, is_active=True
        )
    ...
    def test_plan_types_list(self):
        ...
        basic_plan_data = next(
            plan for plan in data["plans"] if plan["name"] == "basic"
        )
        assert basic_plan_data["display_name"] == "Basic Plan"
        assert basic_plan_data["price"] == "249.00"
    ...
    def test_upgrade_plan_success(self):
        ...
        UserPlan.objects.create(
            user=self.user, plan_type=self.free_plan, is_active=True
        )
    ...
    def test_upgrade_plan_unauthenticated(self):
        ...
        response = self.client.post("/api/plans/upgrade", {"plan_name": "basic"})
...
class TestAssetCreationWithPlanLimits:
    ...
    def setup_method(self):
        ...
        self.basic_plan, created = PlanType.objects.get_or_create(
            name="test_basic_5",
            defaults={"display_name": "Basic Plan", "asset_limit": 5},
        )

        # Create pro plan with unlimited assets
        self.pro_plan, created = PlanType.objects.get_or_create(
            name="pro", defaults={"display_name": "Pro Plan", "asset_limit": -1}
        )
    ...
    def test_asset_creation_unlimited_plan(self):
        ...
        UserPlan.objects.create(user=self.user, plan_type=self.pro_plan, is_active=True)
...
class TestPlanAPIEdgeCases:
    ...
    def test_plan_info_with_inactive_plan(self):
        ...
        UserPlan.objects.create(
            user=self.user, plan_type=inactive_plan, is_active=False
        )
    ...
    def test_plan_info_serialization(self):
        ...
        UserPlan.objects.create(user=self.user, plan_type=plan_type, is_active=True)
