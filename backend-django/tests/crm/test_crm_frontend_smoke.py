...
class CrmFrontendSmokeTests(TestCase):
    ...
    def test_contact_creation_from_asset_page(self):
        ...
        data = {"name": "רות כהן", "email": "rut@example.com", "phone": "050-1234567"}
    ...
    def test_lead_notes_timeline(self):
        ...
        for note_text in notes:
            response = self.client.post(
                f"/api/crm/leads/{lead.id}/add_note", {"text": note_text}
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_lead_status_workflow(self):
        ...
        for status_value in statuses:
            response = self.client.post(
                f"/api/crm/leads/{lead.id}/set_status", {"status": status_value}
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
