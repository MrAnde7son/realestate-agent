...
class CrmIntegrationTests(TestCase):
    ...
    def test_full_contact_lead_workflow(self):
        ...
        response = self.client.post(
            f"/api/crm/leads/{lead_id}/set_status", {"status": "contacted"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # 4. Add notes
        response = self.client.post(
            f"/api/crm/leads/{lead_id}/add_note", {"text": "נפגשנו, ביקשה דוח ממותג"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_lead_status_workflow_integration(self):
        ...
        for from_status, to_status in statuses:
            ...
            response = self.client.post(
                f"/api/crm/leads/{lead.id}/set_status", {"status": to_status}
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
