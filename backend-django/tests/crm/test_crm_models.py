...
class CrmModelsTests(TestCase):
    ...
    def test_contact_str_representation(self):
        ...
        email = self._get_unique_email()
        contact = Contact.objects.create(owner=self.user, name="רות כהן", email=email)

        self.assertEqual(str(contact), f"רות כהן ({email})")

        # Test with no email
        contact_no_email = Contact.objects.create(
            owner=self.user, name="דוד לוי", phone="050-1234567"
        )
    ...
    def test_contact_meta_indexes(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="רות כהן", email=self._get_unique_email()
        )
    ...
    def test_contact_unique_constraints(self):
        ...
        Contact.objects.create(owner=self.user, name="רות כהן", email=email)

        # Try to create duplicate contact with same email for same owner
        with self.assertRaises(Exception):  # Should raise IntegrityError
            with transaction.atomic():
                Contact.objects.create(
                    owner=self.user, name="רות כהן אחרת", email=email
                )
        ...
        other_contact = Contact.objects.create(
            owner=other_user, name="רות כהן אחרת", email=email
        )
    ...
    def test_lead_creation(self):
        ...
        self.assertEqual(
            lead.notes, [{"ts": "2024-01-01T12:00:00Z", "text": "Test note"}]
        )
    ...
    def test_lead_str_representation(self):
        ...
        lead = Lead.objects.create(contact=contact, asset=self.asset, status="new")
    ...
    def test_contact_task_creation_and_completion(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="Contact Task", email=self._get_unique_email("task")
        )

        task = ContactTask.objects.create(
            contact=contact, owner=self.user, title="Follow up call"
        )
    ...
    def test_contact_interaction_creation(self):
        ...
        self.assertEqual(
            str(interaction), f"Interaction({InteractionType.CALL}) for {contact.id}"
        )
    ...
    def test_lead_meta_indexes(self):
        ...
        with connection.cursor() as cursor:
            ...
            self.assertTrue(
                any("last_activity_at" in idx or "last_ac" in idx for idx in indexes)
            )
    ...
    def test_lead_unique_constraints(self):
        ...
        # Create first lead
        Lead.objects.create(contact=contact, asset=self.asset, status="new")

        # Try to create duplicate lead
        with self.assertRaises(Exception):  # Should raise IntegrityError
            Lead.objects.create(contact=contact, asset=self.asset, status="contacted")
    ...
    def test_lead_blank_fields(self):
        ...
        lead = Lead.objects.create(
            contact=contact, asset=self.asset, status="new", notes=[]
        )
    ...
    def test_contact_lead_relationship(self):
        ...
        lead1 = Lead.objects.create(contact=contact, asset=self.asset, status="new")

        lead2 = Lead.objects.create(contact=contact, asset=asset2, status="contacted")
    ...
    def test_asset_lead_relationship(self):
        ...
        contact2 = Contact.objects.create(
            owner=self.user, name="דוד לוי", email=self._get_unique_email()
        )

        # Create leads for different contacts with same asset
        lead1 = Lead.objects.create(contact=contact1, asset=self.asset, status="new")

        lead2 = Lead.objects.create(
            contact=contact2, asset=self.asset, status="contacted"
        )
    ...
    def test_lead_notes_validation(self):
        ...
        lead = Lead.objects.create(
            contact=contact, asset=self.asset, status="new", notes=valid_notes
        )
    ...
    def test_contact_lead_count(self):
        ...
        for i in range(5):
            ...
            Lead.objects.create(contact=contact, asset=asset, status="new")
    ...
    def test_asset_lead_count(self):
        ...
        for i in range(3):
            contact = Contact.objects.create(
                owner=self.user, name=f"רות כהן {i}", email=f"rut{i}@example.com"
            )
            Lead.objects.create(contact=contact, asset=self.asset, status="new")
    ...
    def test_lead_status_choices_validation(self):
        ...
        for i, status in enumerate(valid_statuses):
            ...
            lead = Lead.objects.create(contact=contact, asset=asset, status=status)
    ...
    def test_lead_contact_asset_required(self):
        ...
        with self.assertRaises(Exception):
            Lead.objects.create(asset=self.asset, status="new")

        # Test asset is required
        with self.assertRaises(Exception):
            Lead.objects.create(contact=contact, status="new")

    def test_contact_owner_required(self):
        """Test Contact owner is required"""
        with self.assertRaises(Exception):
            Contact.objects.create(name="רות כהן", email=self._get_unique_email())
