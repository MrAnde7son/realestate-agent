...
class CrmPermissionsTests(TestCase):
    ...
    def test_contact_update_permissions(self):
        ...
        data = {"name": "דוד לוי-כהן", "email": "david@example.com"}
    ...
    def test_lead_custom_actions_permissions(self):
        ...
        response = self.client.post(
            f"/api/crm/leads/{lead.id}/set_status", {"status": "contacted"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Test add_note
        response = self.client.post(
            f"/api/crm/leads/{lead.id}/add_note", {"text": "Test note"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # Test set_status
        response = self.client.post(
            f"/api/crm/leads/{other_lead.id}/set_status", {"status": "contacted"}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        # Test add_note
        response = self.client.post(
            f"/api/crm/leads/{other_lead.id}/add_note", {"text": "Test note"}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_permissions_with_multiple_users(self):
        ...
        for i, user in enumerate(users):
            contact = Contact.objects.create(
                owner=user, name=f"Contact {i}", email=f"contact{i}@example.com"
            )
    ...
    def test_permissions_with_anonymous_user(self):
        ...
        data = {"name": "דוד לוי", "email": "david@example.com"}
