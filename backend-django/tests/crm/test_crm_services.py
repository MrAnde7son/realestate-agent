...
class CrmServicesTests(TestCase):
    ...
    def test_notify_asset_change_with_leads(self, mock_send_email):
        ...
        # Create leads
        Lead.objects.create(contact=contact1, asset=self.asset, status="interested")

        Lead.objects.create(contact=contact2, asset=self.asset, status="negotiating")

        Lead.objects.create(contact=contact3, asset=self.asset, status="new")
    ...
    def test_notify_asset_change_no_email_contacts(self, mock_send_email):
        ...
        # Create lead
        Lead.objects.create(contact=contact, asset=self.asset, status="interested")
    ...
    def test_notify_asset_change_different_assets(self, mock_send_email):
        ...
        Lead.objects.create(contact=contact2, asset=other_asset, status="interested")
    ...
    def test_send_report_to_contact_success(self, mock_send_email, mock_build_pdf):
        ...
        mock_build_pdf.assert_called_once_with(
            payload=report_payload, owner_id=self.user.id
        )
        ...
        self.assertEqual(
            call[1]["attachments"], [("nadlaner-report.pdf", mock_pdf_content)]
        )
    ...
    @patch("crm.services.build_branded_pdf")
    @patch("crm.services.send_email")
    def test_send_report_to_contact_pdf_generation_error(
        self, mock_send_email, mock_build_pdf
    ):
    ...
    @patch("crm.services.build_branded_pdf")
    @patch("crm.services.send_email")
    def test_send_report_to_contact_email_sending_error(
        self, mock_send_email, mock_build_pdf
    ):
    ...
    @patch("crm.services.build_branded_pdf")
    @patch("crm.services.send_email")
    def test_send_report_to_contact_payload_handling(
        self, mock_send_email, mock_build_pdf
    ):
    ...
    def test_send_report_to_contact_performance(self):
        ...
        # Test report sending performance
        with (
            patch("crm.services.build_branded_pdf") as mock_build_pdf,
