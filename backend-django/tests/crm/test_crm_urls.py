...
class CrmUrlsTests(TestCase):
    ...
    def test_interaction_endpoints(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="שרה כהן", email="sara@example.com"
        )
        ...
        detail_url = reverse(
            "contact-interactions-detail", kwargs={"pk": interaction.id}
        )
    ...
    def test_url_consistency(self):
        ...
        contact_urls = [reverse("contacts-list"), reverse("contacts-search")]

        for url in contact_urls:
            self.assertTrue(url.startswith("/api/crm/contacts"))

        # All lead URLs should follow same pattern
        lead_urls = [reverse("leads-list"), reverse("leads-by-asset")]
