...
class CrmViewsTests(TestCase):
    ...
    def test_contact_list_view(self):
        ...
        contact1 = Contact.objects.create(
            owner=self.user, name="רות כהן", email="rut@example.com"
        )

        contact2 = Contact.objects.create(
            owner=self.user, name="דוד לוי", email="david@example.com"
        )

        # Create contact for other user
        other_contact = Contact.objects.create(
            owner=self.other_user, name="שרה גולד", email="sara@example.com"
        )
    ...
    def test_contact_export_view(self):
        ...
        # Create test contacts
        Contact.objects.create(owner=self.user, name="רות כהן", email="rut@example.com")
    ...
    def test_lead_list_view(self):
        ...
        other_lead = Lead.objects.create(
            contact=other_contact, asset=self.asset, status="new"
        )
    ...
    def test_lead_update_view(self):
        ...
        self.assertEqual(
            lead.notes, [{"ts": "2024-01-01T12:00:00Z", "text": "Updated note"}]
        )
    ...
    def test_lead_send_report_action(self):
        ...
        lead = Lead.objects.create(
            contact=contact, asset=self.asset, status="interested"
        )
    ...
    def test_lead_filter_by_status(self):
        ...
        Lead.objects.create(contact=contact, asset=assets[0], status="new")

        Lead.objects.create(contact=contact, asset=assets[1], status="contacted")

        Lead.objects.create(contact=contact, asset=assets[2], status="interested")
    ...
    def test_contact_pagination(self):
        ...
        for i in range(30):
            Contact.objects.create(
                owner=self.user, name=f"Contact {i}", email=f"contact{i}@example.com"
            )
    ...
    def test_cross_user_access(self):
        ...
        other_contact = Contact.objects.create(
            owner=self.other_user, name="דוד לוי", email="david@example.com"
        )
    ...
    def test_contact_task_workflow(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="Task Contact", email="task@example.com"
        )
    ...
    def test_contact_meeting_filters(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="Meeting Contact", email="meeting@example.com"
        )
    ...
    def test_contact_interaction_filters(self):
        ...
        contact = Contact.objects.create(
            owner=self.user, name="Interaction Contact", email="interaction@example.com"
        )
        ...
        response_filtered = self.client.get(
            "/api/crm/interactions", {"contact": contact.id, "type": "call"}
        )
