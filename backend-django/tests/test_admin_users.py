...
from core.admin_views import IsAdminUser
...
class AdminUserViewSetTest(APITestCase):
    ...
    def test_list_users_as_admin(self):
        ...
        )  # admin + regular user + migration users
    ...
    def test_update_user_as_admin(self):
        ...
        response = self.client.patch(
            f"/api/admin/users/{self.regular_user.id}", update_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_deactivate_user(self):
        ...
        response = self.client.post(
            f"/api/admin/users/{self.regular_user.id}/deactivate"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reset_password(self):
        ...
        response = self.client.post(
            f"/api/admin/users/{self.regular_user.id}/reset_password"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_users(self):
        ...
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(
            response.data["results"][0]["email"], "test_regular@example.com"
        )
