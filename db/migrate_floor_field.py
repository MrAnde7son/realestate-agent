            ...
            if database_url.startswith('postgresql'):
                ...
                if row:
                    ...
                    if current_type == 'integer':
                        ...
                        conn.execute(text("ALTER TABLE listings ALTER COLUMN floor TYPE VARCHAR(50)"))
                        conn.commit()
                        print("Floor column successfully converted to VARCHAR(50)")
                    else:
                        print(f"Floor column is already {current_type}, no migration needed")
                else:
                ...
                if floor_column[2].upper() == 'INTEGER':
                    ...
                    # Recreate indexes
                    conn.execute(text("CREATE UNIQUE INDEX ix_listings_listing_id ON listings (listing_id)"))
                    
                    conn.commit()
                    print("Listings table successfully recreated with VARCHAR(50) floor column")
                else:
                    print(f"Floor column is already {floor_column[2]}, no migration needed")
