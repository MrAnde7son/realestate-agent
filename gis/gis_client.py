...
class TelAvivGS:
    ...
    def _safe_json(self, r: requests.Response) -> Dict[str, Any]:
        ...
        raise ArcGISError(
            f"Non-JSON response ({r.status_code}, {ct}); body[:500]={error_preview}"
        )
    ...
    def _query(
        self, layer: str, params: Dict[str, Any], method: str = "GET", retries: int = 1
    ) -> Dict[str, Any]:
        url = f"{self.BASE}/{layer}/query"
        self._logger.info(f"ArcGIS request {method} {url}")
        if method == "POST":
        ...
        if "error" in data:
            self._logger.error(
                "ArcGIS error payload", extra={"payload": data.get("error")}
            )
    ...
    def get_address_coordinates(
        ...
        self._logger.info(
            "Geocoding address",
            extra={"street": street, "house_number": house_number, "like": like},
        )
    ...
    def get_addresses_by_block_parcel(
        ...
        """
        self._logger.info(
            "Looking up addresses by block/parcel",
            extra={"block": block, "parcel": parcel},
        )
        ...
        try:
            ...
            if not parcel_feats:
                self._logger.warning(
                    "Parcel not found", extra={"block": block, "parcel": parcel}
                )
                return []
            ...
            self._logger.info(
                "Found addresses for block/parcel",
                extra={"block": block, "parcel": parcel, "count": len(addresses)},
            )
            return addresses

        except Exception as e:
            self._logger.error(
                "Failed to lookup addresses by block/parcel",
                extra={"block": block, "parcel": parcel, "error": str(e)},
            )
            return []
    ...
    def get_building_permits(
        ...
        self._logger.info(
            "Querying permits around point", extra={"x": x, "y": y, "radius_m": radius}
        )
        params = {
            "f": "pjson",
            "where": "1=1",
        ...
        if download_pdfs and save_dir:
            ...
            for attrs in permits:
                ...
                if not url:
                    self._logger.debug(
                        ...
                        extra={
                            "attrs": {
                                k: attrs.get(k)
                                for k in ("permission_num", "request_num")
                            }
                        },
                    )
                    continue
                ...
                try:
                    self._logger.debug(
                        "Downloading PDF", extra={"url": url, "dest": dest_path}
                    )
                    ...
                    self._logger.warning(
                        "Failed to download PDF", extra={"url": url, "error": str(e)}
                    )
        return permits
    ...
    def get_dangerous_buildings(
        self, x: float, y: float, radius: int = 80
    ) -> List[Dict[str, Any]]:
    ...
    def get_cell_antennas(
        self, x: float, y: float, radius: int = 120
    ) -> List[Dict[str, Any]]:
    ...
    def get_green_areas(
        self, x: float, y: float, radius: int = 150
    ) -> List[Dict[str, Any]]:
        return self._intersects_point(self.L_GREEN, x, y, radius)

    def get_shelters(
        self, x: float, y: float, radius: int = 200
    ) -> List[Dict[str, Any]]:
        return self._intersects_point(self.L_SHELTERS, x, y, radius)

    def get_affordable_housing_projects(
        self, x: float, y: float, radius: int = 500
    ) -> List[Dict[str, Any]]:
    ...
    def get_bike_paths(
        self, x: float, y: float, radius: int = 300
    ) -> List[Dict[str, Any]]:
    ...
    def get_metro_stations(
        self, x: float, y: float, radius: int = 1000
    ) -> List[Dict[str, Any]]:
    ...
    def get_green_amenities(
        self, x: float, y: float, radius: int = 400
    ) -> List[Dict[str, Any]]:
    ...
    def get_youth_entrepreneurship_centers(
        self, x: float, y: float, radius: int = 400
    ) -> List[Dict[str, Any]]:
    ...
    def get_building_privilege_page(
        self, x: float, y: float, save_dir: Optional[str] = "privilege_pages"
        ...
        try:
            ...
            if not blocks or not parcels:
                self._logger.warning(
                    "No blocks or parcels found for location", extra={"x": x, "y": y}
                )
                return None
            ...
            if not block or not parcel:
                self._logger.warning(
                    "Missing block or parcel values",
                    extra={"block": block, "parcel": parcel},
                )
                return None

            self._logger.info(
                "Found block and parcel", extra={"block": block, "parcel": parcel}
            )
            ...
            main_response = requests.get(
                main_url, headers=self.HDRS, timeout=30, allow_redirects=True
            )
            ...
            if "pdf" in main_content_type or main_response.content.startswith(b"%PDF"):
                self._logger.info(
                    "Main page returns PDF directly - single privilege document"
                )
                ...
                try:
                    ...
                    result["pdf_data"].append(
                        {"file_path": dest_path, "data": parsed_pdf, "option": 1}
                    )
                    result["message"] += (
                        " - downloaded 1 privilege page (main page PDF)"
                    )
                    self._logger.info(
                        "Successfully downloaded and parsed main page PDF"
                    )
                except Exception as e:
                    self._logger.warning(f"Failed to parse main page PDF: {e}")
                    result["message"] += (
                        " - downloaded 1 privilege page (parsing failed)"
                    )
                ...
                pattern = re.compile(
                    r'OPTION\s+VALUE\s*=\s*["\']?(\d+)["\']?', re.IGNORECASE
                )
                ...
                if matches:
                    ...
                    try:
                        ...
                        for i in ints:
                            ...
                            self._logger.info(
                                f"Downloading privilege page {i}",
                                extra={"url": opt_url},
                            )
                            ...
                            try:
                                ...
                                # Check if it's a PDF
                                opt_content_type = opt_response.headers.get(
                                    "Content-Type", ""
                                ).lower()
                                if (
                                    "pdf" in opt_content_type
                                    or opt_response.content.startswith(b"%PDF")
                                ):
                                    ...
                                    try:
                                        ...
                                        self._logger.info(
                                            f"Successfully parsed privilege page {i}"
                                        )
                                    except Exception as e:
                                        self._logger.warning(
                                            f"Failed to parse PDF privilege page {i}: {e}"
                                        )
                                else:
                                    self._logger.warning(
                                        f"Privilege page {i} is not a PDF"
                                    )

                            except Exception as e:
                                self._logger.warning(
                                    f"Failed to download privilege page {i}: {e}"
                                )

                        result["message"] += (
                            f" - found {len(ints)} options, downloaded {len(result['pdf_data'])} PDFs"
                        )

                    except Exception as e:
                        self._logger.warning(
                            f"Failed to parse HTML privilege page options: {e}"
                        )
                        # Fall back to trying individual options
                        self._try_individual_privilege_options(
                            block, parcel, save_dir, result
                        )
                else:
            ...
            self._logger.error(
                f"Unexpected error in get_building_privilege_page: {e}", exc_info=True
            )
            return {
                "success": False,
                "error": str(e),
    ...
    def _try_individual_privilege_options(
        self, block: int, parcel: int, save_dir: str, result: Dict[str, Any]
    ) -> None:
        """Try downloading individual privilege page options (opt=1, opt=2, etc.)"""
        self._logger.info(
            "Trying individual privilege page options",
            extra={"block": block, "parcel": parcel},
        )
        ...
        for opt in range(1, 6):
            ...
            try:
                self._logger.info(
                    f"Trying privilege page option {opt}", extra={"url": opt_url}
                )
                opt_response = requests.get(
                    opt_url, headers=self.HDRS, timeout=30, allow_redirects=True
                )
                self._logger.info(
                    f"Response status: {opt_response.status_code}, content-type: {opt_response.headers.get('Content-Type')}, size: {len(opt_response.content)}"
                )
                opt_response.raise_for_status()
                ...
                self._logger.info(
                    f"Content type check: '{opt_content_type}', PDF check: {opt_response.content.startswith(b'%PDF')}"
                )
                if "pdf" in opt_content_type or opt_response.content.startswith(
                    b"%PDF"
                ):
                    self._logger.info(
                        f"PDF detected for option {opt}, proceeding with download"
                    )
                    try:
                        filename = (
                            f"privilege_block_{block}_parcel_{parcel}_opt_{opt}.pdf"
                        )
                        dest_path = os.path.join(save_dir, filename)
                    ...
                    try:
                        ...
                        with pdfplumber.open(dest_path) as pdf:
                            ...
                            self._logger.info(
                                f"Privilege page option {opt} has {text_length} characters"
                            )
                            if text_length < 50:
                                self._logger.info(
                                    f"Privilege page option {opt} appears to be blank ({text_length} chars), skipping"
                                )
                                consecutive_empty += 1
                                ...
                                consecutive_empty = (
                                    0  # Reset counter for non-empty pages
                                )

                    except Exception as e:
                        self._logger.warning(
                            f"Failed to check PDF content for option {opt}: {e}"
                        )
                    ...
                    try:
                        ...
                        parsed_pdf = parse_zchuyot(dest_path)
                        result["pdf_data"].append(
                            {"file_path": dest_path, "data": parsed_pdf, "option": opt}
                        )
                        self._logger.info(
                            f"Successfully downloaded and parsed privilege page option {opt}"
                        )
                        ...
                        self._logger.warning(
                            f"Failed to parse PDF privilege page option {opt}: {e}"
                        )
                else:
                    self._logger.debug(
                        f"Privilege page option {opt} is not a PDF (content-type: {opt_content_type})"
                    )
                    consecutive_empty += 1

            except Exception as e:
                self._logger.debug(
                    f"Failed to download privilege page option {opt}: {e}"
                )
                consecutive_empty += 1
            ...
            if consecutive_empty >= max_consecutive_empty:
                self._logger.info(
                    f"Stopping after {consecutive_empty} consecutive empty pages"
                )
                break

        if result["pdf_data"]:
            result["message"] += (
                f" - downloaded {len(result['pdf_data'])} valid privilege pages"
            )
        else:
    ...
    def get_block_parcel_info(self, x: float, y: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if not blocks or not parcels:
                return {
                    "error": "No blocks or parcels found",
                    "blocks": blocks,
                    "parcels": parcels,
                }
            ...
            if not block or not parcel:
                return {
                    "error": "Missing block or parcel values",
                    "block": block,
                    "parcel": parcel,
                }
    ...
    def _intersects_point(
        ...
        self._logger.info(
            "Querying layer intersects point",
            extra={"layer": layer, "x": x, "y": y, "radius": radius},
        )
        data = self._query(layer, p, method="POST")
        feats = [f.get("attributes", {}) for f in data.get("features", [])]
        self._logger.debug(
            "Layer features fetched", extra={"layer": layer, "count": len(feats)}
        )
        return feats
