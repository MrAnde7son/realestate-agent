...
class ProxyGISClient(ABC):
    ...
    def _identify(
        self, x: float, y: float, layer_id: int, tolerance: int = 9
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in data:
                self._logger.error(
                    "ArcGIS error in response", extra={"error": data.get("error")}
                )
    ...
    def get_building_permits(
        self, x: float, y: float, radius: int = 50, **kwargs
    ) -> List[Dict[str, Any]]:
        """Get building permits (placeholder - returns empty list)."""
        self._logger.warning(
            f"Building permits not yet implemented for {self.__class__.__name__}"
        )
        return []
