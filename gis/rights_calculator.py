...
def calculate_remaining_rights_from_parsed_data(
    parsed_data: Dict[str, Any], parcel_area_sqm: Optional[float] = None
) -> Dict[str, Any]:
    """
    ...
    if percent_building and area:
        ...
        if remaining_percent > 0:
            remaining_rights["calculated_remaining_rights_sqm"] = int(
                (remaining_percent / 100) * area
            )
    ...
    if area and (
        floor_details
        or floor_percentages
        or relative_floor_percentages
        or building_coverage_percentages
        or roof_percentages
        or general_percentages
    ):
        ...
        if relative_floor_percentages and floor_areas:
            for rel_info in relative_floor_percentages:
                ...
                if base_floor in floor_areas:
                    ...
                    remaining_rights[f"relative_{base_floor}_{percentage}%_area"] = int(
                        relative_area
                    )
        ...
        if total_building_rights > 0:
            ...
            remaining_rights["building_coverage_percentages"] = (
                building_coverage_percentages
            )
...
def get_remaining_rights_sqm(
    ...
    """
    remaining_rights = calculate_remaining_rights_from_parsed_data(
        parsed_data, parcel_area_sqm
    )
