...
class DealCache:
    ...
    def get_cached_deals(self, address: str) -> Optional[List[Deal]]:
        ...
        try:
            ...
            if datetime.now() - last_update > timedelta(hours=self.max_age_hours):
                logger.info(
                    f"Cache for {address} is expired (age: {datetime.now() - last_update})"
                )
                return None
            ...
            if error_modal_encountered:
                logger.info(
                    f"Not using cached deals for {address} due to previous error modal encounter"
                )
                return None
    ...
    def store_deals(
        self, address: str, deals: List[Deal], error_modal_encountered: bool = False
    ) -> None:
...
class IncrementalDealCollector:
    ...
    def get_deals_incremental(
        ...
        fresh_deals = self.scraper.get_deals_by_address(
            address, max_age_days=max_age_days, force_refresh=True
        )

        # Check if error modal was encountered during scraping
        error_modal_encountered = getattr(
            self.scraper, "error_modal_encountered", False
        )
        ...
        if should_cache:
            ...
            if max_age_days is not None:
                ...
                # Use force_refresh=True to bypass caching and prevent recursion
                all_deals = self.scraper.get_deals_by_address(
                    address, force_refresh=True
                )
                self.scraper.max_age_days = original_max_age
    ...
    def get_new_deals_since(self, address: str, since_date: datetime) -> List[Deal]:
        ...
        logger.info(
            f"Found {len(new_deals)} new deals since {since_date} for {address}"
        )
        return new_deals
