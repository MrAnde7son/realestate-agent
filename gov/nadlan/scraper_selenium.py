...
class NadlanDealsScraper:
    ...
    def __init__(
        ...
        self.driver = None
        self.current_search_address = (
            None  # Store the current search address for full address construction
        )
        self.error_modal_encountered = (
            False  # Track if error modal was encountered during scraping
        )
    ...
    def _init_driver(self):
        ...
        atexit.register(
            lambda s=self: shutil.rmtree(s._tmp_profile_dir, ignore_errors=True)
            if s._tmp_profile_dir
            else None
        )
        ...
        opts.add_argument(
            f"--user-data-dir={self._tmp_profile_dir}"
        ...
        if chrome_binary:
            ...
            logger.warning(
                "Chrome binary not found in common locations, relying on Selenium Manager"
            )
        ...
        try:
            ...
            if "session not created" in error_msg.lower():
                ...
                logger.error(
                    f"Failed to create Chrome session. {chrome_info}. Error: {error_msg}"
                )
        ...
        try:
            driver.execute_cdp_cmd(
                "Emulation.setTimezoneOverride", {"timezoneId": "Asia/Jerusalem"}
            )
    ...
    def _wait_for_deals_api_call(self, timeout: int = 30) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                    ...
                    try:
                        ...
                        for log in logs:
                            ...
                            if method == "Network.responseReceived":
                                ...
                                if any(
                                    endpoint in url
                                    for endpoint in ["/api/deal", "/deal", "deals"]
                                ):
                                    status = response.get("status", 0)
                                    if status == 200:
                                        logger.info(
                                            f"Deals API call completed successfully: {url}"
                                        )
                                        return True
                                    elif status >= 400:
                                        logger.warning(
                                            f"Deals API call failed with status {status}: {url}"
                                        )
                                        return False
                ...
                if (
                    not network_monitoring_available
                    or time.time() - start_time > timeout * 0.5
                ):
                        ...
                        if len(table_rows) > 0:
                            ...
                            for row in table_rows:
                                ...
                                if len(cells) >= 5:  # Should have at least 5 columns
                                    ...
                                    # Skip header rows
                                    if not any(
                                        keyword in " ".join(cell_texts).lower()
                                        for keyword in [
                                            "מספר סידורי",
                                            "כתובת",
                                            "header",
                            ...
                            if len(data_rows) > 0:
                                logger.info(
                                    f"Found {len(data_rows)} data rows in deals table"
                                )
                                return True
                    except Exception as e:
        ...
        try:
            if self.driver:
                try:
                    ...
                    logger.warning(
                        "Failed to quit Selenium driver cleanly: %s", quit_error
                    )
                except Exception as quit_error:
                    logger.error(
                        "Unexpected error while quitting Selenium driver: %s",
                        quit_error,
                    )
        finally:
            self.driver = None
    ...
    def _extract_neighborhood_from_page(self) -> str:
        ...
        try:
            ...
            neighborhood_section = self.driver.find_elements(
                By.CSS_SELECTOR, ".otherNeighborhoodsSection .mainTitle"
            )
                ...
                if "בשכונה" in title_text:
                    ...
                    if len(parts) > 1:
                        ...
                        if "לפי רחובות" in neighborhood_part:
                            neighborhood_part = neighborhood_part.split("לפי רחובות")[
                                0
                            ].strip()
                        return neighborhood_part

            # Fallback: look for neighborhood in other possible locations
            neighborhood_elements = self.driver.find_elements(
                By.CSS_SELECTOR, "[class*='neighborhood'], [class*='שכונה']"
            )
        ...
        for fmt in date_formats:
            try:
                ...
                if parsed_date.year > current_year:
                    logger.warning(
                        f"Date {date_str} appears to be in the future ({parsed_date.year}), assuming it's {parsed_date.year - 1}"
                    )
    ...
    def _filter_deals_by_age(self, deals: List[Deal]) -> List[Deal]:
        ...
        recent_deals = [
            deal for deal in deals if self._is_deal_recent(deal, cutoff_date)
        ]

        logger.info(
            f"Filtered {len(deals)} deals to {len(recent_deals)} recent deals (last {self.max_age_days} days)"
        )
        return recent_deals
    ...
    def get_neighborhood_info(self, neighbourhood_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise NadlanAPIError(
                f"Failed to get neighborhood info for {neighbourhood_id}: {e}"
            )
        finally:
            self._cleanup_driver()
    ...
    def _extract_deals_from_table(self, neighborhood: str = "") -> List[Deal]:
        ...
        try:
            ...
            tables = self.driver.find_elements(
                By.CSS_SELECTOR, "table#dealsTable, .mainTable, table"
            )
            ...
            for table_idx, main_table in enumerate(tables):
                ...
                for row in rows:
                    try:
                        ...
                        if len(cells) >= 5:  # Should have at least 5 columns
                            ...
                            if any(
                                keyword in " ".join(cell_texts).lower()
                                for keyword in ["מספר סידורי", "כתובת", "header"]
                            ):
                                continue

                            # Extract deal information based on table structure
                            deal_data = {
                                "serial_number": cell_texts[0]
                                if len(cell_texts) > 0
                                else "",
                                "address": self._build_full_address(cell_texts[1])
                                if len(cell_texts) > 1
                                else "",
                                "area": cell_texts[2] if len(cell_texts) > 2 else "",
                                "deal_date": cell_texts[3]
                                if len(cell_texts) > 3
                                else "",
                                "deal_amount": cell_texts[4]
                                if len(cell_texts) > 4
                                else "",
                                "parcelNum": cell_texts[5]
                                if len(cell_texts) > 5
                                else "",
                                "asset_type": cell_texts[6]
                                if len(cell_texts) > 6
                                else "",
                                ...
                                try:
                                    ...
                                    deals.append(
                                        Deal(
                                            ...
                                            deal_amount=deal_data.get(
                                                "deal_amount", ""
                                            ),
                                            deal_date=deal_data.get("deal_date", ""),
                                            rooms=deal_data.get("rooms", ""),
                                            area=deal_data.get("area", ""),
                                            parcel_block=deal_data.get(
                                                "parcelNum", ""
                                            ).split("-")[0]
                                            if deal_data.get("parcelNum")
                                            else "",
                                            parcel_parcel=deal_data.get(
                                                "parcelNum", ""
                                            ).split("-")[1]
                                            if deal_data.get("parcelNum")
                                            and "-" in deal_data.get("parcelNum", "")
                                            else "",
                                            parcel_sub_parcel=deal_data.get(
                                                "parcelNum", ""
                                            ).split("-")[2]
                                            if deal_data.get("parcelNum")
                                            and deal_data.get("parcelNum", "").count(
                                                "-"
                                            )
                                            >= 2
                                            else "",
                ...
                logger.info(
                    f"Extracted {len([d for d in deals if d])} deals from table {table_idx + 1}"
                )
    ...
    def get_deals_by_address(
        ...
        if self.use_cache and self.incremental_collector and not force_refresh:
            try:
                deals = self.incremental_collector.get_deals_incremental(
                    address, force_refresh, max_age_days
                )
                return deals
            except Exception as e:
                logger.warning(
                    f"Incremental collection failed, falling back to direct fetch: {e}"
                )
        ...
        try:
            ...
            if self._navigate_to_deals_via_search(address):
                ...
                try:
                    ...
                    for selector in error_selectors:
                        try:
                            error_modals = self.driver.find_elements(
                                By.CSS_SELECTOR, selector
                            )
                            for modal in error_modals:
                                if modal.is_displayed():
                                    ...
                                    if any(
                                        ...
                                        logger.error(
                                            f"Error modal detected with selector '{selector}': {error_text}"
                                        )
                    ...
                    if error_modal_found:
                        ...
                        raise NadlanAPIError(
                            "Website showed error modal even after refresh: שגיאה בטעינת הנתונים"
                        )
                ...
                if not api_success:
                    logger.warning(
                        "Deals API call may have failed, continuing with data extraction..."
                    )
    ...
    def _navigate_to_deals_via_search(self, search_term: str) -> bool:
        ...
        try:
            ...
            self.driver.execute_script(
                "arguments[0].value = arguments[1];", search_input, search_term
            )
            ...
            if suggestions:
                ...
                for suggestion in suggestions:
                    ...
                    if any(
                        ...
                        for keyword in ["רחוב", "street", "כתובת", "address"]
                    ):
                ...
                if "address" in current_url:
                    ...
                    logger.warning(
                        f"Search '{search_term}' did not lead to address or neighborhood page"
                    )
                    return False
            else:
                ...
                if "address" in current_url or "neighborhood" in current_url:
                    ...
                    logger.warning(
                        "Search did not lead to address or neighborhood page"
                    )
                    return False
    ...
    def _extract_deals_from_current_page(self, neighborhood: str = "") -> List[Deal]:
        ...
        while waited_time < max_wait_time:
            try:
                ...
                if self._check_for_error_modal():
                    raise NadlanAPIError(
                        "Error modal appeared during data loading: שגיאה בטעינת הנתונים"
                    )
                ...
                if len(table_rows) > 0:
                    ...
                    if (
                        deals_data
                        and isinstance(deals_data, list)
                        and len(deals_data) > 0
                    ):
                        logger.info(
                            f"Found deals data in page content: {len(deals_data)} items"
                        )
                ...
                no_deals_msg = self.driver.find_elements(
                    By.CSS_SELECTOR,
                    "[class*='no-deals'], [class*='empty'], [class*='no-data'], .tableSummary",
                )
                if no_deals_msg:
                    for msg in no_deals_msg:
                        if msg.is_displayed() and any(
                            keyword in msg.text.lower()
                            for keyword in ["לא נמצאו", "no deals", "empty", "0 עסקאות"]
                        ):
        ...
        if not deals:
            logger.warning(
                "No deals data found after waiting %s seconds", max_wait_time
            )
    ...
    def _extract_deals_from_all_pages(self, neighborhood: str = "") -> List[Deal]:
        ...
        try:
            ...
            aggressive_early_termination = (
                self.max_age_days > 0 and self.max_age_days < 365
            )

            if aggressive_early_termination:
                logger.info(
                    f"Using aggressive early termination for max_age_days={self.max_age_days}"
                )
                    ...
                    if page_deals:
                        ...
                        if cutoff_date:
                            if self._all_deals_older_than(page_deals, cutoff_date):
                                logger.info(
                                    f"Stopping pagination at page {page_num} - all deals are older than {self.max_age_days} days"
                                )
                                ...
                                aggressive_early_termination
                                and self._most_deals_older_than(
                                    page_deals, cutoff_date, threshold=0.8
                                )
                            ):
                                logger.info(
                                    f"Stopping pagination at page {page_num} - most deals (80%+) are older than {self.max_age_days} days"
                                )
                                # Still add the deals from this page before stopping
                                all_deals.extend(page_deals)
                                break
                        ...
                        if aggressive_early_termination:
                            logger.info(
                                "No deals found on page, stopping pagination due to aggressive early termination"
                            )
                            break
                else:
    ...
    def _most_deals_older_than(
        self, deals: List[Deal], cutoff_date: datetime, threshold: float = 0.8
    ) -> bool:
    ...
    def _get_pagination_info(self) -> dict:
        ...
        try:
            ...
            pagination_elements = self.driver.find_elements(
                By.CSS_SELECTOR, ".tableSummary .pagination .paginate"
            )
            ...
            for i, element in enumerate(pagination_elements):
                ...
                if "/" in text:
                    ...
                    if len(parts) == 2:
                        ...
                        logger.info(
                            f"Found pagination option: {current_page} / {total_pages}"
                        )
            ...
            if pagination_options:
                ...
                for option in pagination_options:
                    ...
                    try:
                        table_summary = option["element"].find_element(
                            By.XPATH,
                            "./ancestor::div[contains(@class, 'tableSummary')]",
                        )
                        ...
                        if any(
                            ...
                            for keyword in ["עסקאות בכתובת", "בכתובת", "address"]
                        ):
                            address_pagination = option
                            logger.info(
                                f"Found address-specific pagination: {option['current_page']} / {option['total_pages']}"
                            )
                        # If it mentions neighborhood or "עסקאות נוספות בשכונה", it's neighborhood-specific
                        elif any(
                            keyword in summary_text
                            for keyword in [
                                "עסקאות נוספות בשכונה",
                                "בשכונה",
                                "neighborhood",
                            ]
                        ):
                            ...
                            logger.info(
                                f"Found neighborhood-specific pagination: {option['current_page']} / {option['total_pages']}"
                            )
                    except Exception as e:
                ...
                if address_pagination:
                    logger.info(
                        f"Using address-specific pagination: {address_pagination['current_page']} / {address_pagination['total_pages']}"
                    )
                    return address_pagination
                elif neighborhood_pagination:
                    logger.info(
                        f"Using neighborhood-specific pagination: {neighborhood_pagination['current_page']} / {neighborhood_pagination['total_pages']}"
                    )
                    return neighborhood_pagination
                else:
                    # Fallback: choose the pagination with the fewest pages (likely address-specific)
                    best_option = min(
                        pagination_options, key=lambda x: x["total_pages"]
                    )
                    logger.info(
                        f"Fallback: selected pagination with fewest pages: {best_option['current_page']} / {best_option['total_pages']}"
                    )
                    return best_option
            ...
            for i, next_button in enumerate(next_buttons):
                if next_button.is_displayed() and not next_button.get_attribute(
                    "disabled"
                ):
            ...
            for i, next_button in enumerate(next_buttons):
                try:
                    if next_button.is_displayed() and not next_button.get_attribute(
                        ...
                        logger.info(
                            f"Attempting to click next button {i + 1} for page {page_num}"
                        )
                        ...
                        try:
                            ...
                            if new_pagination and new_pagination["current_page"] > 1:
                                logger.info(
                                    f"Regular click successful for button {i + 1}, now on page {new_pagination['current_page']}"
                                )
                                return True
                            else:
                                logger.debug(
                                    f"Regular click didn't change page for button {i + 1}"
                                )

                        except Exception as click_error:
                            logger.debug(
                                f"Regular click failed for button {i + 1}: {click_error}"
                            )
                            ...
                            try:
                                ...
                                if (
                                    new_pagination
                                    and new_pagination["current_page"] > 1
                                ):
                                    logger.info(
                                        f"JavaScript click successful for button {i + 1}, now on page {new_pagination['current_page']}"
                                    )
                                    return True
                                else:
                                    logger.debug(
                                        f"JavaScript click didn't change page for button {i + 1}"
                                    )

                            except Exception as js_error:
                                logger.debug(
                                    f"JavaScript click failed for button {i + 1}: {js_error}"
                                )
                                continue
    ...
    def _get_neighborhood_info_selenium(self, neighbourhood_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            name_element = self.driver.find_element(
                By.CSS_SELECTOR, "h1, .neighborhood-name, .title"
            )
            ...
            logger.warning(
                "Failed to read neighborhood name for %s: %s",
                neighbourhood_id,
                name_error,
            )
            info["neigh_name"] = f"Neighborhood {neighbourhood_id}"
        ...
        try:
            ...
            details = self.driver.find_elements(
                By.CSS_SELECTOR, ".neighborhood-details, .info, .details"
            )
            ...
            logger.debug(
                "Failed to extract neighborhood details for %s: %s",
                neighbourhood_id,
                details_error,
            )
