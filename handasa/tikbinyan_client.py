...
class TikbinyanClient(ABC):
    ...
    def _find_street_id(
        self, street_name: str, site_id: Optional[str] = None
    ) -> Optional[str]:
        ...
        for street in streets:
            ...
            if (
                normalized_search in normalized_label
                or normalized_label in normalized_search
            ):
                return street.get("v")
    ...
    def _get_gush_file(
        self, block: str, parcel: Optional[str] = None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"Successfully fetched gush file for block={block}, parcel={parcel}"
                )
                return response
            else:
                logger.warning(
                    f"Failed to fetch gush file: HTTP {response.status_code}"
                )
        except requests.RequestException as e:
    ...
    def _parse_gush_file_response(
        self, html: str, block: str, parcel: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        logger.debug(
            f"Found {len(building_ids)} building IDs in gush file: {building_ids}"
        )
            ...
            for req_id in set(request_ids):
                ...
                if row_match:
                    ...
                    if len(cells) >= 3:
                        ...
                        documents.append(
                            self._normalize_document(document, source_name)
                        )

        if documents:
            logger.info(
                f"Successfully parsed {len(documents)} documents from gush file for block={block}, parcel={parcel}"
            )
        else:
            logger.warning(
                f"No documents found in gush file for block={block}, parcel={parcel}"
            )

        return documents

    def _get_unified_file(
        self, block: str, parcel: Optional[str] = None
    ) -> Optional[requests.Response]:
        ...
        try:
            ...
            if response.status_code == 200:
                self._fix_response_encoding(response)
                logger.debug(
                    f"Successfully fetched unified file for block={block}, parcel={parcel}"
                )
                return response
            else:
                logger.warning(
                    f"Failed to fetch unified file: HTTP {response.status_code}"
                )
        except requests.RequestException as e:
    ...
    def _parse_unified_file_response(
        self, html: str, block: str, parcel: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        logger.debug(
            f"Found {len(building_ids)} building IDs in unified file: {building_ids}"
        )
        ...
        if documents:
            logger.info(
                f"Successfully parsed {len(documents)} documents from unified file for block={block}, parcel={parcel}"
            )
        else:
            logger.warning(
                f"No documents found in unified file for block={block}, parcel={parcel}"
            )
    ...
    def _normalize_document(
        ...
        """
        document_type = (
            row.get("document_type") or row.get("type") or row.get("title") or ""
        )
        status = row.get("status") or row.get("stage") or ""
        permit_num = (
            row.get("permit_num")
            or row.get("permission_num")
            or row.get("permit_number")
        )
        request_num = row.get("request_num") or row.get("request_number")
        document_date = (
            row.get("document_date") or row.get("date") or row.get("issue_date")
        )
    ...
    def _classify_document_type(
        descriptor: Optional[str], row: Optional[Dict[str, Any]] = None
    ) -> str:
        ...
        if any(
            ...
            for keyword in ["היתר", "permit", "בקשה", "request"]
        ):
            if "מילולי" in normalized_lower or "verbal" in normalized_lower:
                return "permit_verbal"
            if "תכנית" in normalized_lower or "plan" in normalized_lower:
        ...
        if any(
            keyword in normalized_lower
            for keyword in ["מפת מדידה", "technical", "drawing"]
        ):
            return "technical_drawing"

        if any(
            keyword in normalized_lower
            for keyword in ["תכנית אדריכלית", "architectural"]
        ):
...
class BatYamTikbinyanClient(TikbinyanClient):
    ...
    def _get_by_building_id(self, building_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Failed to fetch building {building_id}: HTTP {response.status_code}"
                )
        except requests.RequestException as e:
            logger.error(
                f"Failed to fetch building {building_id} from Bat Yam: {e}",
                exc_info=True,
            )

        return []

    def _get_by_block_parcel(
        self, block: str, parcel: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get building info by block and parcel."""
        if not parcel:
            logger.warning(
                "Parcel number is required for block/parcel search in Bat Yam"
            )
            return []
        ...
        if gush_response:
            gush_docs = self._parse_gush_file_response(
                gush_response.text, block, parcel
            )
        ...
        if unified_response:
            unified_docs = self._parse_unified_file_response(
                unified_response.text, block, parcel
            )
    ...
    def _search_buildings_by_street(
        self, street_id: str, house_number: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if buildings:
                    logger.info(
                        f"Found {len(buildings)} buildings for street {street_id}, house {house_number}"
                    )
                    ...
                    for building in buildings:
                        ...
                        if isinstance(building, dict):
                            ...
                            logger.debug(
                                f"Building dict: {building}, extracted ID: {building_id}"
                            )
                    ...
                    logger.warning(
                        f"No buildings found in response for street_id={street_id}, house_number={house_number}"
                    )
        ...
        logger.warning(
            f"Could not find buildings for street_id={street_id}, house_number={house_number}"
        )
        return []

    def _parse_api_response(
        self, data: Dict[str, Any], building_id: str
    ) -> List[Dict[str, Any]]:
        ...
        if isinstance(data, dict):
            ...
            building_data = (
                data.get("building") or data.get("data") or data.get("result") or data
            )

            # Extract permits/documents
            permits = (
                building_data.get("permits")
                or building_data.get("documents")
                or building_data.get("files")
                or []
            )
    ...
    def _parse_building_page(self, html: str, building_id: str) -> List[Dict[str, Any]]:
        ...
        if request_ids:
            logger.info(
                f"Found {len(request_ids)} requests/permits in HTML for building {building_id}"
            )
            ...
            for req_id in set(request_ids):  # Use set to avoid duplicates
                ...
                row_match = re.search(
                    row_pattern, tbody_html, re.DOTALL | re.IGNORECASE
                )
                ...
                document = {
                    ...
                    or f"Request {req_id}",  # Raw description for classification
                    "building_id": building_id,
                    ...
                    "meta": {
                        ...
                        "all_cells": cell_texts[
                            :6
                        ],  # Store first 6 cells for debugging
                    },
                }
            ...
            if documents:
                logger.info(
                    f"Successfully parsed {len(documents)} documents from HTML for building {building_id}"
                )
                return documents
        ...
        if not documents:
            logger.warning(
                f"Could not extract JSON data or parse HTML tables for building page {building_id}"
            )
...
class HerzliyaTikbinyanClient(TikbinyanClient):
    ...
    def _get_by_block_parcel(
        ...
        if not parcel:
            logger.warning(
                "Parcel number is required for block/parcel search in Herzliya"
            )
            return []
    ...
    def _parse_building_page(self, html: str, building_id: str) -> List[Dict[str, Any]]:
        ...
        for pattern in json_patterns:
            ...
            for match in matches:
                try:
                    data = json.loads(
                        match if match.startswith("{") else match.replace("'", '"')
                    )
                    return self._parse_api_response(data, building_id)
                except (json.JSONDecodeError, ValueError):
                    continue
        logger.warning(
            f"Could not extract JSON data from building page for {building_id}"
        )
        return documents
...
class RamatGanTikbinyanClient(TikbinyanClient):
    ...
    def _get_by_block_parcel(
        ...
        if not parcel:
            logger.warning(
                "Parcel number is required for block/parcel search in Ramat Gan"
            )
            return []
