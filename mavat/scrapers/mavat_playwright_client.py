...
class MavatPlaywrightClient:
    ...
    def search_plans(
        ...
        if not self.page:
            raise RuntimeError(
                "Browser not started. Call start() first or use context manager."
            )
        ...
        try:
            ...
            try:
                ...
                self.page.wait_for_selector(
                    ".result-item, .plan-item, [data-plan-id]", timeout=10000
                )

                # Extract plan information
                plan_elements = self.page.query_selector_all(
                    ".result-item, .plan-item, [data-plan-id]"
                )

                for element in plan_elements[:limit]:
                    try:
                        plan_id = (
                            element.get_attribute("data-plan-id")
