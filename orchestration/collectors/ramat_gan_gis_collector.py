"""GIS data collector for Ramat Gan."""

import logging
from typing import Any, Dict, Optional, Tuple

from gis.ramat_gan_gis_client import RamatGanGIS
...
class RamatGanGISCollector(BaseCollector):
    """Collector for Ramat Gan GIS data."""

    def __init__(self, client: Optional[RamatGanGIS] = None) -> None:
        self.client = client or RamatGanGIS()
        ...
        **kwargs
        ...
        if not x or not y:
            if not search_street:
                raise ValueError("RamatGanGISCollector requires coordinates (x_itm, y_itm) or a street name")
    ...
    def _geocode(self, street: str, house_number: int, city: str = "") -> Tuple[float, float]:
        """Geocode an address to coordinates using Ramat Gan GIS."""
        logger.info(f"Geocoding address via Ramat Gan GIS: {street} {house_number}, {city}")
        try:
            x, y = self.client.get_address_coordinates(street, house_number, like=True)
            logger.info(f"Geocoded to coordinates via Ramat Gan GIS: {x}, {y}")
            return x, y
        except Exception as e:
            logger.error(f"Ramat Gan GIS geocoding failed: {e}")
