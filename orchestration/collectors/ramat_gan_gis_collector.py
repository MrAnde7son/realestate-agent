...
class RamatGanGISCollector(BaseCollector):
    ...
    def __init__(self, client: Optional[RamatGanGIS] = None, govmap_client: Optional[GovMapClient] = None) -> None:
        ...
        if block and parcel and not search_street:
            ...
            if not x or not y:
                if not search_street:
                    raise ValueError("RamatGanGISCollector requires at least a street name or block/parcel")
