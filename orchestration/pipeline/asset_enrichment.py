...
def _extract_listing_type(listing_data: Any) -> Tuple[Optional[str], Optional[str]]:
    ...
    listing_type_value = listing_data.get("listing_type") or listing_data.get(
        "listingType"
    )
...
def _compute_ppm_weight(entry: Dict[str, Any]) -> float:
    ...
    if published_days is None and isinstance(entry.get("meta"), dict):
        published_days = entry["meta"].get("published_days") or entry["meta"].get(
            "publishedDays"
        )

    recency_weight = _compute_recency_weight(
        entry.get("deal_date"), published_days=published_days
    )
...
def _calculate_relevance_score(
    entry: Dict[str, Any], asset_total_area: Optional[float]
) -> float:
    ...
    recency_weight = _compute_recency_weight(
        entry.get("deal_date"), entry.get("published_days")
    )
    ...
    if asset_total_area and asset_total_area > 0:
        ...
        if entry_area > 0:
            area_ratio = min(asset_total_area, entry_area) / max(
                asset_total_area, entry_area
            )
...
def _filter_outliers_iqr(
    ppm_values: List[float], multiplier: float = 1.5, one_sided: bool = True
) -> List[float]:
...
def _calculate_base_ppm_and_value(
    ...
    if ppm_values:
        ...
        top_ppm_values = [
            entry["ppm"] for entry in sorted_entries[:5] if entry.get("ppm") is not None
        ]
    ...
    if asset_total_area and asset_total_area > 0:
        ...
        # Cannot calculate reliable base value without total area
        logger.warning(
            f"[PRICE_MODEL] Cannot calculate base_value without total_area. "
            f"Asset area is {asset_total_area}. Skipping base_value calculation to avoid unreliable results."
        )
...
def _finish_level_adjustment(asset: Any) -> float:
    ...
    if _safe_bool(getattr(asset, "renovated", False)) or finish_level in (
        "high",
        "premium",
    ):
        return 0.15
...
def _calculate_adjustment_data_completeness(
    asset: Any, feature_vector: FeatureVector
) -> float:
    ...
    area = _safe_numeric(getattr(asset, "total_area", None)) or _safe_numeric(
        getattr(asset, "area", None)
    )
    ...
    has_potential_data = (
        ...
        or (isinstance(meta, dict) and meta.get("planning_potential"))
        or _safe_bool(getattr(asset, "permit_urban_renewal", False))
    )
    checks.append(("potential", has_potential_data, 0.8))

    # New build factor - important for price
    has_new_build_data = year_built is not None or (
        isinstance(meta, dict)
        and (meta.get("new_project") or meta.get("tama38_completed"))
    )
    checks.append(("new_build", has_new_build_data, 0.7))

    # Attachments factor - affects attractiveness
    has_attachments_data = (
        _safe_numeric(getattr(asset, "parking_spaces", None)) is not None
        or _safe_bool(getattr(asset, "storage_room", False))
        or _safe_numeric(getattr(asset, "balcony_area", None)) is not None
        or (
            isinstance(meta, dict)
            and (
                _safe_numeric(meta.get("garden_area")) is not None
                or _safe_numeric(meta.get("roof_space")) is not None
            )
        )
    )
    ...
    has_finish_data = _safe_bool(getattr(asset, "renovated", False)) or (
        isinstance(meta, dict)
        and (meta.get("finish_level") or meta.get("renovation_state"))
    )
    checks.append(("finish_level", has_finish_data, 0.6))

    # Elevator factor - affects attractiveness
    has_elevator_data = (
        _safe_bool(getattr(asset, "elevator", False))
        or _safe_numeric(getattr(asset, "floor", None)) is not None
    )
    checks.append(("elevator", has_elevator_data, 0.6))

    # Proximity factor - environment score component
    has_proximity_data = isinstance(meta, dict) and (
        _safe_numeric(meta.get("distance_to_transit_m")) is not None
        or _safe_numeric(meta.get("distance_to_beach_m")) is not None
        or _safe_numeric(meta.get("distance_to_sea_m")) is not None
    )
    checks.append(("proximity", has_proximity_data, 0.8))

    # Noise factor - environment score component
    has_noise_data = isinstance(meta, dict) and (
        _safe_numeric(meta.get("nuisance_index")) is not None
        or _safe_numeric(meta.get("distance_to_main_road_m")) is not None
    )
...
def _build_feature_vector(asset: Any) -> FeatureVector:
    size = _safe_numeric(getattr(asset, "total_area", None)) or _safe_numeric(
        getattr(asset, "area", None)
    )
...
def _update_distance_to_sea(
    ...
    if distance is not None:
        logger.debug(
            f"Asset {asset_id}: Calculated distance to sea = {distance:.2f} meters"
        )
...
def update_asset_with_collected_data(
    asset_id: int,
    block: str,
    parcel: str,
    govmap_autocomplete_data: Dict[str, Any],
    ...
    tikbinyan_archive: Optional[List[Dict[str, Any]]] = None,
) -> None:
    ...
    with asset_update_phase("basic_fields", asset_id):
        ...
        if govmap_addresses:
            ...
            if addr_x is not None and addr_y is not None:
                try:
                    ...
                    if (
                        not current_coords_valid
                        or asset.lat is None
                        or asset.lon is None
                    ):
                        asset.lat = addr_lat
                        asset.lon = addr_lon
                        logger.debug(
                            "Asset %s coordinates set from GovMap parcel addresses WGS84 (lat=%.8f lon=%.8f)",
                            asset_id,
                            addr_lat,
                            addr_lon,
                        )
                    ...
                    logger.warning(
                        "Failed to convert GovMap parcel address coordinates for asset %s: %s",
                        asset_id,
                        e,
                    )
        ...
        if (
            (not current_coords_valid or asset.lat is None or asset.lon is None)
            and lon_wgs84 is not None
            and lat_wgs84 is not None
        ):
            if is_valid_israel_coords(lat_wgs84, lon_wgs84):
                asset.lat = lat_wgs84
                asset.lon = lon_wgs84
                logger.debug(
                    "Asset %s coordinates set from GovMap WGS84 (lat=%.8f lon=%.8f)",
                    asset_id,
                    lat_wgs84,
                    lon_wgs84,
                )
            ...
            try:
                lon_wgs84_gis, lat_wgs84_gis = itm_to_wgs84(
                    gis_data.get("x"), gis_data.get("y")
                )
                asset.lat = lat_wgs84_gis
                asset.lon = lon_wgs84_gis
                logger.debug(
                    "Asset %s coordinates converted from GIS ITM -> WGS84", asset_id
                )
            except Exception:
                logger.exception(
                    "Failed to convert GIS coordinates for asset %s (ITM x=%.2f, y=%.2f); skipping invalid coordinate assignment",
                    asset_id,
                    gis_data.get("x"),
                    gis_data.get("y"),
                )
        ...
        if govmap_data.get("addresses") and not getattr(asset, "street", None):
            ...
            if addresses:
                ...
                logger.info(
                    f"Updated asset {asset_id} with street from GovMap: {asset.street}, number: {asset.number}, city: {asset.city}"
                )
        ...
        if govmap_data.get("city") and not getattr(asset, "city", None):
            ...
            logger.info(
                f"Updated asset {asset_id} with city from GovMap data: {asset.city}"
            )
        ...
        if gis_data.get("addresses") and not getattr(asset, "street", None):
            ...
            if addresses:
                ...
                asset.city = first_address.get("city", "")
                logger.info(
                    f"Updated asset {asset_id} with street from GIS: {asset.street}, number: {asset.number}, city: {asset.city}"
                )
        ...
        if gis_data.get("city") and not getattr(asset, "city", None):
            ...
            logger.info(
                f"Updated asset {asset_id} with city from GIS data: {asset.city}"
            )

        # Also update the normalized_address field if GovMap provided a detailed address
        if (
            govmap_data.get("address")
            and govmap_data.get("address") != f"גוש {block} חלקה {parcel}"
        ):
            # Only update if it's not the generic block/parcel format
            asset.normalized_address = govmap_data.get("address")
            logger.info(
                f"Updated asset {asset_id} normalized_address field from GovMap: {asset.normalized_address}"
            )

        if getattr(asset, "street", None) and getattr(asset, "number", None):
            asset.normalized_address = (
                f"{asset.street} {asset.number}"
                + (
                    f" דירה {asset.apartment}"
                    if getattr(asset, "apartment", None)
                    else ""
                )
                + (f" {asset.city}" if getattr(asset, "city", None) else "")
            )
        if not asset.meta:
            asset.meta = {}
    ...
    with asset_update_phase("process_gis", asset_id):
        if gis_data:
            logger.info(
                f"Asset {asset_id}: GIS processing - gis_data exists: {bool(gis_data)}, keys: {list(gis_data.keys()) if gis_data else 'None'}"
            )
            asset.meta["gis_data"] = {
        ...
        if not is_tel_aviv:
            logger.info(
                f"Asset {asset_id}: Non-Tel Aviv city - using multi-city GIS data"
            )
        else:
            if handasa_archive:
            ...
            try:
                ...
                if x is not None and y is not None:
                    ...
                    privilege_data = gis_client.get_building_privilege_page(
                        x, y, save_dir="privilege_pages"
                    )
                    if privilege_data and isinstance(privilege_data, dict):
                        ...
                        if parsed_items:
                            ...
                            asset.meta["privilege_page_refreshed_at"] = (
                ...
                logger.debug(
                    "Privilege page acquisition failed for asset %s",
                    asset_id,
                    exc_info=True,
                )
    ...
    with asset_update_phase("process_government", asset_id):
        if gov_data:
            ...
            _extract_neighborhood_from_transactions(
                asset, gov_data.get("transactions", [])
            )
    ...
    with asset_update_phase("normalize_listings", asset_id):
        ...
        if listings and not normalized_listings:
            logger.debug(
                "All listings dropped while normalizing listing data for asset %s",
                asset_id,
            )
        if normalized_listings:
            # Store all listings (both Yad2 and Madlan) in meta
            asset.meta["yad2_listings"] = (
                normalized_listings  # Keep key for backward compatibility
            )
    ...
    with asset_update_phase("calculate_building_rights", asset_id):
        ...
        if gis_data:
            ...
            logger.info(
                f"Asset {asset_id}: Skipping building rights calculation (no GIS data)"
            )
    ...
    with asset_update_phase("create_documents_and_plans", asset_id):
        ...
        logger.info(
            f"Asset {asset_id}: Processing municipal archive - is_tel_aviv={is_tel_aviv}, archive_size={len(municipal_archive)}"
        )
        if municipal_archive:
            logger.info(
                f"Asset {asset_id}: First 3 documents in archive: {[{k: v for k, v in list(doc.items())[:5]} for doc in municipal_archive[:3]]}"
            )
        _create_documents_and_plans(
            asset, gis_data, gov_data, plans, mavat_plans, municipal_archive
        )
    ...
    logger.info(
        "Updated asset %s with block=%s, parcel=%s, subparcel=%s",
        asset_id,
        block,
        parcel,
        subparcel,
...
def create_asset_snapshot(asset_id: int, results: List[Any]) -> None:
    ...
    try:
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                if source in ("yad2", "madlan"):
                    ...
                    elif isinstance(listing_data, dict) and listing_data.get(
                        "listing_id"
                    ):
        ...
        # Create snapshot
        Snapshot.objects.create(asset=asset, payload=payload, ppsqm=asset.price_per_sqm)
...
def _process_gis_data(
    asset, gis_data, x_itm: Optional[float] = None, y_itm: Optional[float] = None
):
    ...
    """
    logger.info(
        f"Asset {asset.id}: _process_gis_data called with gis_data keys: {list(gis_data.keys()) if gis_data else 'None/empty'}"
    )
    ...
    if gis_data.get("parcels"):
        ...
        if parcels:
            ...
            asset.set_property(
                "parcelArea",
                parcel_data.get("ms_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "parcelRegisteredArea",
                parcel_data.get("ms_shetach_rashum"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "parcelStatus",
                parcel_data.get("t_status_hesder"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "parcelAccuracy",
                parcel_data.get("k_dargat_diyuk"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
    ...
    if gis_data.get("blocks"):
        ...
        if blocks:
            ...
            asset.set_property(
                "blockArea",
                block_data.get("ms_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "blockRegisteredArea",
                block_data.get("ms_shetach_rashum"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "blockTotalParcels",
                block_data.get("ms_mispar_chelkot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "blockStatus",
                block_data.get("t_status_hesder"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "blockLastUpdate",
                block_data.get("tr_idkun_acharon"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
        ...
        if addresses:
            ...
            if address_data.get("x") and address_data.get("y"):
                try:
                    ...
                    lon_wgs84, lat_wgs84 = itm_to_wgs84(
                        address_data.get("x"), address_data.get("y")
                    )
                    asset.set_property(
                        "lat", lat_wgs84, source="GIS", url="https://www.govmap.gov.il/"
                    )
                    asset.set_property(
                        "lon", lon_wgs84, source="GIS", url="https://www.govmap.gov.il/"
                    )
                except Exception as e:
    ...
    if gis_data.get("city"):
        asset.set_property(
            "city", gis_data.get("city"), source="GIS", url="https://www.govmap.gov.il/"
        )
    ...
    if gis_data.get("noise"):
        ...
        if noise_levels:
            max_noise = max(
                [n.get("isov3", 0) for n in noise_levels if isinstance(n, dict)]
            )
            asset.set_property(
                "noiseLevel", max_noise, source="GIS", url="https://www.govmap.gov.il/"
            )
    ...
    if gis_data.get("permits"):
        ...
        if permits:
            ...
            asset.set_property(
                "permitRequestNum",
                recent_permit.get("request_num"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitPermissionNum",
                recent_permit.get("permission_num"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            ...
            asset.set_property(
                "permitBuildingNum",
                recent_permit.get("building_num"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitHousingUnits",
                recent_permit.get("yechidot_diyur"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitCommercialArea",
                recent_permit.get("mischar_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitResidentialArea",
                recent_permit.get("megurim_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitResidentialUnits",
                recent_permit.get("megurim_yechidot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            ...
            asset.set_property(
                "permitPublicArea",
                recent_permit.get("mivney_tzibur_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitParkingArea",
                recent_permit.get("melonaut_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitParkingUnits",
                recent_permit.get("melonaut_yechidot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitSmallApartments",
                recent_permit.get("dirot_ktanot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitUnifiedHousingArea",
                recent_permit.get("diyur_meuchad_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitUnifiedHousingUnits",
                recent_permit.get("diyur_meuchad_yechidot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitAccessibleApartments",
                recent_permit.get("dirot_haskara"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitPublicBuiltArea",
                recent_permit.get("tziburi_banuy_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitTotalArea",
                recent_permit.get("sach_shetach"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitMavatPlanNum",
                recent_permit.get("mispar_tochnit_mavat"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitParkingRoomsCalculated",
                recent_permit.get("melonaut_rooms_mechushav"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitFullUtilization",
                recent_permit.get("sw_mimush_male"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitSubjectType",
                recent_permit.get("sug_nose"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitProcess",
                recent_permit.get("maslul"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitRightsNotification",
                recent_permit.get("sw_niyud_zchuyot"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitRepartition",
                recent_permit.get("sw_repartzelatzya"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            asset.set_property(
                "permitUrbanRenewal",
                recent_permit.get("sw_hitchadshut_ironit"),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            ...
            if recent_permit.get("permission_date"):
                try:
                    permit_date = datetime.fromtimestamp(
                        recent_permit["permission_date"] / 1000
                    )
                    asset.set_property(
                        "permitDate",
            ...
            if recent_permit.get("open_request"):
                try:
                    open_request_date = datetime.fromtimestamp(
                        recent_permit["open_request"] / 1000
                    )
                    asset.set_property(
                        "permitOpenRequestDate",
                        open_request_date.date().isoformat(),
                        source="GIS",
                        url="https://www.govmap.gov.il/",
                    )
                except Exception as e:
            ...
            if recent_permit.get("tr_hathalat_bniya"):
                try:
                    construction_start_date = datetime.fromtimestamp(
                        recent_permit["tr_hathalat_bniya"] / 1000
                    )
                    asset.set_property(
                        "permitConstructionStartDate",
                        construction_start_date.date().isoformat(),
                        source="GIS",
                        url="https://www.govmap.gov.il/",
                    )
                except Exception as e:
            ...
            asset.set_property(
                "permitStatus",
                recent_permit.get("building_stage", ""),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
    ...
    if gis_data.get("shelters"):
        ...
        if shelters and len(shelters) > 0:
            distance_values = [
                s.get("distance")
                for s in shelters
                if isinstance(s, dict) and s.get("distance") is not None
            ]
            if distance_values:
    ...
    if green_indicators:
        asset.set_property(
            "openSpacesNearby",
            ", ".join(green_indicators),
            source="GIS (calculated)",
            url="https://www.govmap.gov.il/",
        )
        asset.set_property(
            "greenScore",
            "גבוה" if len(green_indicators) >= 2 else "בינוני",
            source="GIS (calculated)",
            url="https://www.govmap.gov.il/",
        )
    else:
    ...
    if gis_data.get("metro_stations"):
        ...
        if metro_stations and len(metro_stations) > 0:
            distances = [
                ...
                if isinstance(s, dict) and s.get("distance")
            ]
            if distances:
                min_distance = min(distances)
                asset.set_property(
                    "metroStationDistanceM",
                    min_distance,
                    source="GIS",
                    url="https://www.govmap.gov.il/",
                )
    ...
    if parking_lots:
        ...
        public_parking = [
            p for p in parking_lots if isinstance(p, dict) and p.get("type") == "public"
        ]
        if public_parking:
            asset.set_property(
                "publicParkingLotsCount",
                len(public_parking),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
    ...
    if schools:
        asset.set_property(
            "schoolsCount", len(schools), source="GIS", url="https://www.govmap.gov.il/"
        )
        distances = [
            s.get("distance")
            for s in schools
    ...
    if gis_data.get("affordable_housing"):
        ...
        if affordable_housing:
            asset.set_property(
                "affordableHousingProjectsCount",
                len(affordable_housing),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
    ...
    if bike_paths:
        ...
        asset.set_property(
            "hasBikePaths", True, source="GIS", url="https://www.govmap.gov.il/"
        )
    ...
    if gis_data.get("soil_contamination"):
        ...
        if soil_contamination and len(soil_contamination) > 0:
            asset.set_property(
                "soilContaminationSitesCount",
                len(soil_contamination),
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
            distances = [
                s.get("distance")
                for s in soil_contamination
                if isinstance(s, dict) and s.get("distance")
            ]
            if distances:
                min_distance = min(distances)
                asset.set_property(
                    "nearestSoilContaminationDistanceM",
                    min_distance,
                    source="GIS",
                    url="https://www.govmap.gov.il/",
                )
    ...
    if gis_data.get("green_amenities"):
        ...
        if green_amenities:
            ...
            playgrounds = [
                g
                for g in green_amenities
                if isinstance(g, dict)
                and "playground" in str(g.get("type", "")).lower()
            ]
            asset.set_property(
                "playgroundsCount",
                len(playgrounds),
    ...
    if medical_facilities:
        ...
        distances = [
            ...
            if isinstance(m, dict) and m.get("distance")
        ]
        if distances:
            min_distance = min(distances)
            asset.set_property(
                "nearestMedicalFacilityDistanceM",
                min_distance,
                source="GIS",
                url="https://www.govmap.gov.il/",
            )
    ...
    if community_facilities:
        asset.set_property(
            "communityFacilitiesCount",
            len(community_facilities),
            source="GIS",
            url="https://www.govmap.gov.il/",
        )
    ...
    if medical_centers:
        ...
        if distances:
            min_distance = min(distances)
            existing_distance = asset.get_property_value(
                "nearestMedicalFacilityDistanceM"
            )
    ...
    if health_funds:
        ...
        distances = [
            h.get("distance")
            for h in health_funds
            if isinstance(h, dict) and h.get("distance")
        ]
        if distances:
            min_distance = min(distances)
            existing_distance = asset.get_property_value(
    ...
    if pharmacies:
        ...
        distances = [
            p.get("distance")
            for p in pharmacies
            if isinstance(p, dict) and p.get("distance")
        ]
        if distances:
            min_distance = min(distances)
            existing_distance = asset.get_property_value(
    ...
    if tama38_areas:
        asset.set_property(
            "tama38KeyArea", True, source="GIS", url="https://www.govmap.gov.il/"
        )
        asset.set_property(
            "tama38KeyAreasCount",
            len(tama38_areas),
    ...
    if road_works:
        ...
        asset.set_property(
            "hasActiveRoadWorks", True, source="GIS", url="https://www.govmap.gov.il/"
        )
    ...
    soil_contamination_count = (
        asset.get_property_value("soilContaminationSitesCount") or 0
    )
    if soil_contamination_count and soil_contamination_count > 0:
        nearest_contamination = asset.get_property_value(
            "nearestSoilContaminationDistanceM"
        )
    ...
    if schools_count and schools_count > 0:
        ...
        if nearest_school and nearest_school <= 300:
            potential_indicators.append(
                f"קרוב לבתי ספר ({schools_count} בתי ספר וגנים)"
            )
    ...
    affordable_housing_count = (
        asset.get_property_value("affordableHousingProjectsCount") or 0
    )
    if affordable_housing_count and affordable_housing_count > 0:
        potential_indicators.append(
            f"פרויקטי דיור מועדף באזור ({affordable_housing_count}) - שינוי דמוגרפי צפוי"
        )

    if potential_indicators:
        asset.set_property(
            "investmentPotential",
            "; ".join(potential_indicators),
            source="GIS (calculated)",
            url="https://www.govmap.gov.il/",
        )
        ...
        asset.set_property(
            "investmentPotential",
            "אין אינדיקטורים מיוחדים לפוטנציאל",
            source="GIS (calculated)",
            url="https://www.govmap.gov.il/",
        )
        asset.set_property(
...
def _calculate_building_rights(asset, gis_data):
    ...
    try:
        ...
        if privilege_data_list:
            ...
            if isinstance(privilege_data_list, list):
                    ...
                    if privilege_data:
                        try:
                            ...
                            remaining_rights_sqm = get_remaining_rights_sqm(
                                privilege_data, area_for_calculation
                            )
                            ...
                            logger.debug(
                                f"Failed to calculate rights from privilege page: {e}"
                            )
                            continue
                ...
                try:
                    ...
                    remaining_rights_sqm = get_remaining_rights_sqm(
                        privilege_data_list, area_for_calculation
                    )
                    if remaining_rights_sqm:
                        source = "GIS (privilege page)"
                except Exception as e:
        ...
        asset.set_property(
            "remainingRightsSqm",
            remaining_rights_sqm,
            source=source,
            url="https://www.govmap.gov.il/",
        )
        asset.set_property(
            "mainRightsSqm",
            int(area_for_calculation),
            source="GIS (calculated)",
            url="https://www.govmap.gov.il/",
        )
        # Only calculate service rights if remaining_rights_sqm is not None
        service_rights_sqm = (
            int(remaining_rights_sqm * 0.1)
            if remaining_rights_sqm is not None
            else None
        )
        asset.set_property(
            "serviceRightsSqm",
...
def _process_government_data(asset, gov_data):
    ...
    if gov_data.get("decisive"):
        ...
        if decisive:
            ...
            asset.set_property(
                "appraisalValue",
                latest_appraisal.get("appraised_value"),
                source="מנהל התכנון",
                url="https://www.gov.il/",
            )
            asset.set_property(
                "appraisalDate",
                latest_appraisal.get("appraisal_date"),
                source="מנהל התכנון",
                url="https://www.gov.il/",
            )
...
def _extract_neighborhood_from_transactions(asset, transactions):
    ...
    if not filtered_transactions:
        logger.debug(
            f"No matching transactions found for asset {asset.id} location, skipping neighborhood extraction"
        )
        return
        ...
        if isinstance(transaction, dict):
            ...
            if neighborhood and isinstance(neighborhood, str) and neighborhood.strip():
                neighborhood_counts[neighborhood] = (
                    neighborhood_counts.get(neighborhood, 0) + 1
                )

    if neighborhood_counts:
        # Use the most common neighborhood from matching transactions
        most_common_neighborhood = max(neighborhood_counts.items(), key=lambda x: x[1])[
            0
        ]
        asset.neighborhood = most_common_neighborhood
        logger.info(
            f"Updated asset {asset.id} neighborhood from {len(filtered_transactions)} matching transactions: {most_common_neighborhood}"
        )
...
def _filter_transactions_by_asset_location(asset, transactions):
    ...
    if not asset_block or not asset_parcel:
        logger.debug(
            f"Asset {asset.id} missing block/parcel info, skipping neighborhood extraction from transactions"
        )
        return []
...
def _process_rami_plans(asset, plans):
    ...
    if plans:
        ...
        active_plans = [
            p for p in plans if p.get("status") and "פעיל" in p.get("status", "")
        ]
        if active_plans:
            ...
            asset.set_property(
                ...
                source="RAMI",
                url="https://rami.gov.il/",
            )
            asset.set_property(
                "planActive", True, source="RAMI", url="https://rami.gov.il/"
            )
        else:
            asset.set_property(
                "planActive", False, source="RAMI", url="https://rami.gov.il/"
            )
        ...
        asset.set_property(
            "additionalPlanRights",
            additional_rights_text,
            source="RAMI (calculated)",
            url="https://rami.gov.il/",
        )
        ...
        for category in [
            ...
            if items:
                ...
                if first_item.get("Value"):
                    asset.set_property(
                        f"govmap_{category.lower()}_name",
                        first_item.get("Value"),
                        source="GovMap Autocomplete",
                        url="https://www.govmap.gov.il/",
                    )
                if first_item.get("Text"):
                    asset.set_property(
                        f"govmap_{category.lower()}_text",
                        first_item.get("Text"),
                        source="GovMap Autocomplete",
                        url="https://www.govmap.gov.il/",
                    )
                    ...
                    if gis_field_name:
                        ...
                        if gis_field_name == "publicTransport":
                            ...
                            if layer_key == "bus_stations":
                                public_transport_indicators.append(
                                    f"תחנות אוטובוס ({count})"
                                )
                        ...
                        if (
                            entities
                            and hasattr(asset, "lat")
                            and hasattr(asset, "lon")
                            and asset.lat
                            and asset.lon
                        ):
                            try:
                                ...
                                for entity in entities:
                                    ...
                                    if centroid and len(centroid) == 2:
                                        ...
                                        entity_lon, entity_lat = itm_to_wgs84(
                                        ...
                                        distance = haversine_distance(
                                            asset.lat, asset.lon, entity_lat, entity_lon
                                        )
                                ...
                                logger.debug(
                                    f"Failed to calculate distances for {layer_key}: {e}"
                                )
                    else:
                        # For layers without GIS equivalents, store with GovMap prefix
                        asset.set_property(
                            f"govmap_{layer_key}_count",
                            count,
                            source="GovMap",
                            url="https://www.govmap.gov.il/",
                        )
                ...
                if layer_data:
                    ...
                    if gis_field_name:
                        ...
                        if gis_field_name == "publicTransport":
                            if layer_key == "bus_stations":
                                public_transport_indicators.append(
                                    f"תחנות אוטובוס ({len(layer_data)})"
                                )
                            elif layer_key == "train_stations":
                                public_transport_indicators.append(
                                    f"תחנות רכבת ({len(layer_data)})"
                                )
                        elif gis_field_name == "tama38KeyArea":
                            # Urban renewal maps to TAMA 38 key areas (like GIS)
                            asset.set_property(
...
def _create_django_records_from_collected_data(
    asset,
    govmap_autocomplete_data,
    govmap_data,
    gis_data,
    ...
    if listings and not normalized_listings:
        logger.debug(
            "All listings dropped while normalizing listings for Django source records on asset %s",
            asset.id,
        )
            ...
            if listing_id:
                ...
                if DjangoListing is not None:
                    ...
                    recent_deal_value = next(
                        (
                            value
                            for value in recent_deal_candidates
                    ...
                    ad_type_value = (
                        ...
                        or listing_meta.get("ad_type")
                        or listing_meta.get("adType")
                        or (listing_meta.get("raw") or {}).get("adType")
                        if isinstance(listing_meta.get("raw"), dict)
                        else None
                    )
                    ...
                    if not ad_type_value:
                        contact_info = (
                            ...
                            or listing.get("contactInfo")
                            or {}
                        )
                        features = listing.get("features") or {}
                        has_agency = (
                            bool(contact_info.get("agent"))
                            or bool(features.get("agency"))
                            or bool(
                                listing_meta.get("customer", {}).get("agencyName")
                                if isinstance(listing_meta.get("customer"), dict)
                                else None
                            )
                        )
                        if has_agency:
                    ...
                    listing_defaults = {
                        ...
                        or listing_meta.get("status")
                        or "active",
                        "price": listing.get("price"),
                        "rooms": listing.get("rooms"),
                        "area": listing.get("area"),
                        "address": listing.get("address"),
                        "recent_deal": bool(recent_deal_value)
                        if recent_deal_value is not None
                        else False,
                        ...
                        or listing.get("contactName")
                        or (
                            (listing.get("contact_info") or {}).get("name")
                            if isinstance(listing.get("contact_info"), dict)
                            else None
                        ),
                        "contact_phone": (
                            listing.get("contact_phone")
                            or listing.get("contactPhone")
                            or (
                                (listing.get("contact_info") or {}).get("phone")
                                if isinstance(listing.get("contact_info"), dict)
                                else None
                            )
                            or (
                                (listing.get("contact_info") or {}).get("brokerPhone")
                                if isinstance(listing.get("contact_info"), dict)
                                else None
                            )
                        ),
                        ...
                        or listing.get("videoUrl")
                        or listing.get("video"),
                    }
                    try:
                        listing_obj, created_listing = (
                            DjangoListing.objects.get_or_create(
                                source=listing_source,
                                external_id=str(listing_id),
                        ...
                        if not created_listing:
                            updates = _collect_field_updates(
                                listing_obj, listing_defaults
                            )
                            if updates:
                                listing_obj.save(
                                    update_fields=list(dict.fromkeys(updates))
                                )
        ...
        for appraisal in gov_data.get("decisive", []):
            ...
            if appraisal_id:
                _safe_source_record_create(
                    ...
                    defaults={
                        "title": appraisal.get("title")
                        or f"שומה החלטית {appraisal_id}",
                        "raw": appraisal,
                    },
                )
    ...
    if gov_data and gov_data.get("transactions"):
        for transaction in gov_data.get("transactions", []):
            ...
            if not deal_id or not str(deal_id).strip():
                ...
                deal_id = (
                    f"fallback_{hashlib.md5(transaction_str.encode()).hexdigest()[:16]}"
                )
            ...
            try:
                ...
                transaction_obj = RealEstateTransaction.objects.filter(
                    deal_id=str(deal_id)[:100]
                ).first()
            except Exception as e:
                logger.warning(
                    f"Failed to create transaction with deal_id {deal_id[:50]}...: {e}"
                )
                continue
...
def _populate_asset_fields_from_listings(asset, normalized_listings):
    ...
    yad2_listings = [
        listing for listing in normalized_listings if _is_yad2_listing(listing)
    ]
    madlan_listings = [
        listing for listing in normalized_listings if _is_madlan_listing(listing)
    ]
        ...
        if isinstance(meta, dict):
            ...
            if isinstance(madlan_raw, dict):
                ...
                if isinstance(address_details_raw, dict):
                    candidate = address_details_raw.get(
                        "neighbourhood"
                    )  # Note: British spelling
                    if isinstance(candidate, str) and candidate.strip():
                        return candidate.strip()
            ...
            if isinstance(raw_meta, dict):
                ...
                if isinstance(raw_neighborhood, dict):
                    candidate = raw_neighborhood.get("text") or raw_neighborhood.get(
                        "name"
                    )
                    if isinstance(candidate, str) and candidate.strip():
                        return candidate.strip()
                ...
                if isinstance(address, dict):
                    ...
                    if isinstance(raw_neighborhood, dict):
                        candidate = raw_neighborhood.get(
                            "text"
                        ) or raw_neighborhood.get("name")
                        if isinstance(candidate, str) and candidate.strip():
                            return candidate.strip()
                    elif isinstance(raw_neighborhood, str) and raw_neighborhood.strip():
                        return raw_neighborhood.strip()
    ...
    def _is_listing_commercial(listing_data):
        ...
        is_commercial = listing_data.get("is_commercial") or listing_data.get(
            "isCommercial"
        )
    ...
    if asset_block and asset_parcel:
        listing_block = (
            str(
                _safe_get(best_listing, "parcel_block")
                or _safe_get(best_listing.get("meta", {}), "parcel_block")
                or _safe_get(best_listing.get("meta", {}), "block")
                or ""
            )
            .strip()
            .lower()
        ...
        listing_parcel = (
            str(
                _safe_get(best_listing, "parcel_parcel")
                or _safe_get(best_listing.get("meta", {}), "parcel_parcel")
                or _safe_get(best_listing.get("meta", {}), "parcel")
                or ""
            )
            .strip()
            .lower()
        ...
        if listing_block and listing_parcel:
            if asset_block != listing_block or asset_parcel != listing_parcel:
                ...
                logger.debug(
                    "[ASSET_FIELDS] Listing address matches but block/parcel differs, skipping neighborhood extraction"
                )
            else:
                logger.debug(
                    "[ASSET_FIELDS] Listing matches asset block/parcel and address, extracting neighborhood"
                )
        else:
            # Listing doesn't have block/parcel (typical for Yad2/Madlan), but address matches exactly
            logger.debug(
                "[ASSET_FIELDS] Listing matches exact address (no block/parcel in listing), extracting neighborhood"
            )
    ...
    if should_extract_neighborhood:
        ...
        if listing_neighborhood and not asset.neighborhood:
            ...
            logger.debug(
                "[ASSET_FIELDS] Set neighborhood from listing: %s", asset.neighborhood
            )
    ...
    if listing_price is not None:
        ...
        if listing_type_normalized == "rent":
            ...
            if hasattr(asset, "rent_price") and not getattr(asset, "rent_price", None):
                ...
                logger.debug(
                    "[ASSET_FIELDS] Set rent_price from listing: %s", asset.rent_price
                )
        else:
            if not asset.price:
    ...
    if listing_net_area and not asset.area:
        ...
        default_url = (
            ...
            if listing_source == "yad2"
            else "https://www.madlan.co.il/"
        )
        asset.set_property(
            "area",
            listing_net_area,
            source=source_name,
            url=best_listing.get("url", default_url),
        )
    ...
    if listing_total_area and not asset.total_area:
        ...
        update_fields.add("meta")
        logger.debug(
            "[ASSET_FIELDS] Set total_area from listing total_size: %s",
            asset.total_area,
        )
    ...
    if asset.price and (asset.total_area or asset.area):
        ...
        if area_to_use > 0:
            ...
            logger.debug(
                "[ASSET_FIELDS] Calculated price_per_sqm: %s", asset.price_per_sqm
            )
    ...
    if not asset.building_type:
        ...
        if listing_property_type:
            ...
            logger.debug(
                "[ASSET_FIELDS] Set building_type from listing: %s", asset.building_type
            )
    ...
    if not getattr(asset, "contact_name", None):
        contact_name = (
            ...
            or (
                (best_listing.get("contact_info") or {}).get("name")
                if isinstance(best_listing.get("contact_info"), dict)
                else None
            )
        )
        if contact_name:
    ...
    if not getattr(asset, "contact_phone", None):
        contact_phone = (
            ...
            or (
                (best_listing.get("contact_info") or {}).get("phone")
                if isinstance(best_listing.get("contact_info"), dict)
                else None
            )
            or (
                (best_listing.get("contact_info") or {}).get("brokerPhone")
                if isinstance(best_listing.get("contact_info"), dict)
                else None
            )
        )
    ...
    listing_type = (
        listing_type_value
        or best_listing.get("listing_type")
        or best_listing.get("listingType")
    )
    ad_type = best_listing.get("ad_type") or best_listing.get("adType")
    if (
        listing_type
        and hasattr(asset, "listing_type")
        and not getattr(asset, "listing_type", None)
    ):
    ...
    price_dropped = best_listing.get("priceDropped") or best_listing.get(
        "price_dropped"
    )
    previous_price = best_listing.get("previousPrice") or best_listing.get(
        "previous_price"
    )
    ...
    building_class = best_listing.get("buildingClass") or best_listing.get(
        "building_class"
    )
    general_condition = best_listing.get("generalCondition") or best_listing.get(
        "general_condition"
    )

    # Investment data
    investment_yield = best_listing.get("investmentYield") or best_listing.get(
        "investment_yield"
    )
    approximate_rent = best_listing.get("approximateRent") or best_listing.get(
        "approximate_rent"
    )
    commute_time = best_listing.get("commuteTime") or best_listing.get("commute_time")

    # Date fields
    published_days = best_listing.get("publishedDays") or best_listing.get(
        "published_days"
    )
    date_posted = best_listing.get("datePosted") or best_listing.get("date_posted")

    # Tags
    tag_best_school = best_listing.get("tagBestSchool") or best_listing.get(
        "tag_best_school"
    )
    tag_safety = best_listing.get("tagSafety") or best_listing.get("tag_safety")
    tag_family_friendly = best_listing.get("tagFamilyFriendly") or best_listing.get(
        "tag_family_friendly"
    )
    tag_light_rail = best_listing.get("tagLightRail") or best_listing.get(
        "tag_light_rail"
    )
    tag_park_access = best_listing.get("tagParkAccess") or best_listing.get(
        "tag_park_access"
    )
    tag_quiet_street = best_listing.get("tagQuietStreet") or best_listing.get(
        "tag_quiet_street"
    )
    ...
    if price_dropped is not None:
        primary_listing_source["priceDropped"] = (
            bool(price_dropped) if isinstance(price_dropped, bool) else price_dropped
        )
    if previous_price is not None:
        primary_listing_source["previousPrice"] = previous_price
    if shelter is not None:
        primary_listing_source["shelter"] = (
            bool(shelter) if isinstance(shelter, bool) else shelter
        )
    if accessibility is not None:
        primary_listing_source["accessibility"] = (
            bool(accessibility) if isinstance(accessibility, bool) else accessibility
        )
    ...
    if tag_best_school is not None:
        primary_listing_source["tagBestSchool"] = (
            bool(tag_best_school)
            if isinstance(tag_best_school, bool)
            else tag_best_school
        )
    if tag_safety is not None:
        primary_listing_source["tagSafety"] = (
            bool(tag_safety) if isinstance(tag_safety, bool) else tag_safety
        )
    if tag_family_friendly is not None:
        primary_listing_source["tagFamilyFriendly"] = (
            bool(tag_family_friendly)
            if isinstance(tag_family_friendly, bool)
            else tag_family_friendly
        )
    if tag_light_rail is not None:
        primary_listing_source["tagLightRail"] = (
            bool(tag_light_rail) if isinstance(tag_light_rail, bool) else tag_light_rail
        )
    if tag_park_access is not None:
        primary_listing_source["tagParkAccess"] = (
            bool(tag_park_access)
            if isinstance(tag_park_access, bool)
            else tag_park_access
        )
    if tag_quiet_street is not None:
        primary_listing_source["tagQuietStreet"] = (
            bool(tag_quiet_street)
            if isinstance(tag_quiet_street, bool)
            else tag_quiet_street
        )
    if tag_commute is not None:
        primary_listing_source["tagCommute"] = (
            bool(tag_commute) if isinstance(tag_commute, bool) else tag_commute
        )
...
def _is_listing_commercial(listing_data: Dict[str, Any]) -> bool:
    ...
    return (
        listing_data.get("ad_type", "") == "commercial"
        or listing_data.get("adType", "") == "commercial"
    )
...
def _calculate_market_metrics(asset, listings, gov_data):
    ...
    try:
        ...
        if listing_dicts:
            for listing in listing_dicts:
                ...
                if price and area and area > 0:
                    ...
                    published_days = listing.get("published_days") or listing.get(
                        "publishedDays"
                    )
                    if published_days is None and isinstance(listing.get("meta"), dict):
                        published_days = listing["meta"].get(
                            "published_days"
                        ) or listing["meta"].get("publishedDays")
        ...
        if ppm_data:
            ...
            # IMPORTANT: Only calculate model price if we have total_area, otherwise results are unreliable
            if base_ppm is not None:
                ...
                if base_value is None:
                    # Cannot calculate reliable model price without total area
                    logger.warning(
                        f"[PRICE_MODEL] asset={asset_id} Cannot calculate model price without total_area. "
                        f"Asset area is {asset_total_area}. Skipping model price calculation to avoid unreliable results. "
                        f"PPM={base_ppm:.0f} is still available for reference."
                    )
                else:
                ...
                simple_avg_ppm_all = (
                    sum([d["ppm"] for d in ppm_data]) / len(ppm_data)
                    if ppm_data
                    else None
                )
                simple_avg_base_value_all = (
                    simple_avg_ppm_all * asset_total_area
                    if (simple_avg_ppm_all and asset_total_area)
                    else None
                )
                simple_avg_model_price_all = (
                    int(simple_avg_base_value_all * adjustment_multiplier)
                    if simple_avg_base_value_all
                    else None
                )

                if base_value is not None:
                    adjusted_model_price = metrics.get("modelPrice")
                    logger.info(
                    ...
                    if price_value and adjusted_model_price > 0:
                        metrics["priceGapPct"] = round(
                            (
                                (price_value - adjusted_model_price)
                                / adjusted_model_price
                            )
                            * 100,
                            2,
                        )
            ...
            data_completeness = _calculate_adjustment_data_completeness(
                asset, feature_vector
            )
            ...
            if areas and asset_area_value:
                avg_area = sum(areas) / len(areas)
                if avg_area > 0:
                    metrics["deltaVsAreaPct"] = round(
                        ((asset_area_value - avg_area) / avg_area) * 100, 2
                    )
        ...
        logger.error(
            "Failed to calculate market metrics for asset %s: %s",
            getattr(asset, "id", "?"),
            e,
        )


def _calculate_planning_legal_analysis(
    asset, gis_data: Dict[str, Any], gov_data: Dict[str, Any]
) -> None:
    ...
    try:
        ...
        if update_fields:
            ...
            logger.debug(
                "[PLANNING_LEGAL] asset=%s analysis=%s",
                asset.id,
                {
        ...
        logger.error(
            "Failed to calculate planning and legal analysis for asset %s: %s",
            getattr(asset, "id", "?"),
            e,
        )


def _create_documents_and_plans(
    asset, gis_data, gov_data, plans, mavat_plans, handasa_archive
):
    ...
    try:
        ...
        system_user, created = User.objects.get_or_create(
            ...
            defaults={"first_name": "System", "last_name": "User", "is_active": False},
        )
        ...
        if gis_data and gis_data.get("permits"):
            logger.info(
                f"Asset {asset.id}: Creating {len(gis_data.get('permits', []))} documents from GIS permits"
            )
            _create_documents_from_permits(
                asset, gis_data.get("permits", []), source="GIS"
            )
        ...
        if handasa_archive:
            ...
            if handasa_archive:
                ...
                logger.info(
                    f"Asset {asset.id}: Checking source from first document: '{first_doc_source}'"
                )
            ...
            all_doc_types = [
                doc.get("document_type", "MISSING") for doc in handasa_archive
            ]
            logger.info(
                f"Asset {asset.id}: All document types in archive ({len(handasa_archive)} docs): {all_doc_types}"
            )

            # Filter for permit documents - check both document_type and document_category
            municipal_permits = [
                doc
                for doc in handasa_archive
                if (doc.get("document_type") or "").startswith("permit")
                or doc.get("document_category") == "permit"
                or (
                    doc.get("request_num") or doc.get("permission_num")
                )  # If it has request/permit num, treat as permit
            ]
            logger.info(
                f"Asset {asset.id}: Found {len(municipal_permits)} permit documents from {len(handasa_archive)} total documents (source: {source})"
            )
            if municipal_permits:
                logger.info(
                    f"Asset {asset.id}: Permit document details: {[{k: v for k, v in doc.items() if k in ['document_type', 'document_category', 'title', 'permission_num', 'request_num', 'source']} for doc in municipal_permits[:3]]}"
                )
                _create_documents_from_permits(asset, municipal_permits, source=source)
            else:
                logger.warning(
                    f"Asset {asset.id}: No permit documents found. All document types: {all_doc_types}"
                )
                logger.warning(
                    f"Asset {asset.id}: Document categories: {[doc.get('document_category', 'MISSING') for doc in handasa_archive]}"
                )
                logger.warning(
                    f"Asset {asset.id}: Request/permit nums: {[doc.get('request_num') or doc.get('permission_num') for doc in handasa_archive]}"
                )
        ...
        if gis_data:
            ...
            if local_plans:
                _create_documents_from_gis_plans(
                    asset, local_plans, "local", system_user
                )

            if city_plans:
                _create_documents_from_gis_plans(
                    asset, city_plans, "citywide", system_user
                )
...
def _create_documents_from_gis_plans(asset, plans, scope: str, system_user):
    ...
    for index, plan in enumerate(plans, start=1):
        ...
        effective_date = _extract_plan_date(
            _first_nonempty(
                plan.get("tr_matan_tokef"),
                plan.get("effective_date"),
                plan.get("effectiveDate"),
            )
        )
        ...
        normalized_id = re.sub(r"[^0-9A-Za-z]+", "_", plan_number_str).strip(
            "_"
        ) or str(index)
        ...
        inferred = _infer_mime_from_name(filename) or _infer_mime_from_name(
            external_url
        )
        if inferred:
        ...
        document_payload = {
            ...
            "file_size": int(plan.get("ms_mismach") or 0)
            if str(plan.get("ms_mismach") or "").strip().isdigit()
            else 0,
            "filename": filename,
            "file_path": "",
...
def _normalize_permit_document_fields(
    permit: Dict[str, Any], source: str, fallback_index: int
    ...
    if source_key in ("handasa", "tikbinyan"):
        meta = (
            permit.get("meta") if isinstance(permit.get("meta"), dict) else dict(permit)
        )
        external_id_raw = (
        ...
        title = (
            permit.get("title") or f"היתר {external_id}"
            if external_id
            else "היתר בנייה"
        )
        description = (
            permit.get("description")
            or meta.get("TlvMPEngDocumentType", "")
            or permit.get("title", "")
        )
        ...
        building_stage = permit.get("building_stage") or meta.get("building_stage")
        if building_stage:
    ...
    external_id = str(
        _normalize_identifier(permission_num or request_num or f"gis_{fallback_index}")
    )
    ...
    return {
        ...
        "source": "GIS",
        "document_date": _convert_unix_timestamp_to_date(
            permit.get("permission_date", 0)
        ),
        "meta": permit,
...
def _create_documents_from_rami_plans(asset, plans):
    ...
    for plan in plans or []:
        ...
        external_id = (
            f"rami_plan_{plan_number}" if plan_number else f"rami_plan_{created + 1}"
        )
        document_payload = {
            "title": f"תכנית רמ״י - {plan_name}"
            if plan_name
            else (f"תכנית רמ״י {plan_number}" if plan_number else "תכנית רמ״י"),
            ...
            "filename": f"rami_plan_{plan_number}.pdf"
            if plan_number
            else f"rami_plan_{created + 1}.pdf",
            "file_path": "",
            "mime_type": "application/pdf",
            "meta": {
...
def _calculate_public_buildings(gis_data: Dict[str, Any], asset) -> str:
    ...
    for permit in permits:
        if isinstance(permit, dict):
            ...
            if any(
                keyword in building_stage or keyword in tochen_bakasha
                for keyword in [
                    "ציבורי",
                    "ממשלתי",
                    "עירייה",
...
def _calculate_parking_availability(gis_data: Dict[str, Any], asset) -> str:
    ...
    for permit in permits:
        if isinstance(permit, dict):
            ...
            tochen_bakasha = permit.get("tochen_bakasha", "").lower()
            hakala_tosefet_achuz_shetach = permit.get(
                "hakala_tosefet_achuz_shetach", ""
            )
            if any(
                keyword in tochen_bakasha for keyword in ["חניה", "חניון", "מקום חניה"]
            ):
                parking_permits.append(permit)
            elif (
                hakala_tosefet_achuz_shetach
                and float(hakala_tosefet_achuz_shetach or 0) > 0
            ):
                parking_permits.append(permit)
...
def _calculate_nearby_projects(gis_data: Dict[str, Any], asset) -> str:
    ...
    tama38_permits = [
        ...
        for p in recent_permits
        if p.get("sw_tama_38")
        or p.get("sw_tama_38_chadash")
        or p.get("sw_tama_38_tosefet")
    ]
    if tama38_permits:
...
def _calculate_additional_plan_rights(plans: List[Dict[str, Any]], asset) -> str:
    ...
    for plan in plans:
        if isinstance(plan, dict):
            ...
            if any(
                keyword in plan_name.lower()
                for keyword in ["הרחבה", "תוספת", "הגדלה", "פיתוח"]
            ):
                additional_rights.append(
                    f"תכנית {plan.get('planNumber', '')}: {plan_name}"
                )
...
def _calculate_public_obligations(
    plans: List[Dict[str, Any]], permits: List[Dict[str, Any]]
) -> str:
    ...
    for plan in plans:
        if isinstance(plan, dict):
            ...
            if any(
                keyword in plan_name.lower()
                for keyword in ["חובה", "תרומה", "השקעה", "תשתית"]
            ):
    ...
    for permit in permits:
        if isinstance(permit, dict):
            tochen_bakasha = permit.get("tochen_bakasha", "").lower()
            if any(
                keyword in tochen_bakasha
                for keyword in ["חובה", "תרומה", "תשתית", "שירותים"]
            ):
                obligations.append(
                    f"היתר {permit.get('permission_num', '')}: חובות ציבוריות"
                )
...
def asset_update_phase(phase: str, asset_id: int | None = None):
    ...
    try:
        ...
        logger.debug(
            "[ASSET_UPDATE] ✔ phase=%s asset_id=%s duration_ms=%d", phase, asset_id, dt
        )
    except Exception as exc:  # noqa: BLE001 - we purposefully capture & log all
        dt = (time.perf_counter() - t0) * 1000
        logger.exception(
            "[ASSET_UPDATE] ✖ phase=%s asset_id=%s duration_ms=%d error=%s",
            phase,
            asset_id,
            dt,
            exc,
...
def find_outdated_price_per_sqm_assets(
    ...
    assets = (
        Asset.objects.filter(price__isnull=False, price__gt=0)
        .exclude(total_area__isnull=True)
    ...
    for asset in assets:
        ...
        if price and total_area and total_area > 0:
            ...
            if stored_ppm is None or abs(stored_ppm - expected_ppm) > (
                expected_ppm * threshold_pct
            ):
                outdated.append(
                    {
                        "id": asset.id,
...
def recalculate_price_model(asset_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[RECALCULATE_PRICE] Starting price model recalculation for asset {asset_id}"
        )
        ...
        gov_data = {"transactions": transaction_dicts}
        ...
        if market_metrics is None:
            market_metrics = {}
            logger.warning(
                f"[RECALCULATE_PRICE] No market_metrics found after calculation for asset {asset_id}"
            )

        adjusted_ppm = (
            market_metrics.get("adjustedPricePerSqm")
            if isinstance(market_metrics, dict)
            else None
        )
        base_ppm = (
            market_metrics.get("avgPricePerSqm")
            if isinstance(market_metrics, dict)
            else None
        )
        ...
        if asset.price and (asset.total_area or asset.area):
            ...
            if area_to_use > 0:
                asset.price_per_sqm = int(asset.price / area_to_use)
                logger.info(
                    f"[RECALCULATE_PRICE] Updated price_per_sqm to {asset.price_per_sqm} (price={asset.price}, area={area_to_use})"
                )
        ...
        logger.info(
            f"[RECALCULATE_PRICE] Successfully recalculated price model for asset {asset_id}"
        )
        ...
        return {
            ...
            "metrics": {
                ...
                "base_model_price": market_metrics.get("baseModelPrice")
                if market_metrics
                else None,
        ...
        logger.error(
            f"[RECALCULATE_PRICE] Error recalculating price model for asset {asset_id}: {e}",
            exc_info=True,
        )
