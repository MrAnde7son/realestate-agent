...
def calculate_planning_legal_analysis(
    asset, gis_data: Dict[str, Any], tabu_data: Dict[str, Any] = None
) -> PlanningLegalAnalysis:
    """
    ...
    privilege_page_data = (
        asset.meta.get("privilege_page_data", []) if hasattr(asset, "meta") else []
    )

    # Calculate rights usage percentage
    analysis.rights_usage_pct = _calculate_rights_usage_percentage(
        asset, gis_data, privilege_page_data
    )

    # Calculate legal restrictions
    analysis.legal_restrictions = _calculate_legal_restrictions(
        asset, gis_data, tabu_data, privilege_page_data
    )

    # Calculate urban renewal potential
    analysis.urban_renewal_potential = _calculate_urban_renewal_potential(
        asset, gis_data
    )

    # Calculate betterment levy
    analysis.betterment_levy = _calculate_betterment_levy(
        asset, gis_data, privilege_page_data
    )
...
def _calculate_rights_usage_percentage(
    asset, gis_data: Dict[str, Any], privilege_page_data: List[Dict[str, Any]] = None
) -> Optional[float]:
    """
    ...
    try:
        ...
        for permit in permits:
            if isinstance(permit, dict):
                ...
                current_building_area += (
                    residential_area + commercial_area + public_area + parking_area
                )
        ...
        max_building_area = _get_maximum_building_area(
            gis_data, parcel_area, privilege_page_data
        )
...
def _get_maximum_building_area(
    gis_data: Dict[str, Any],
    parcel_area: float,
    privilege_page_data: List[Dict[str, Any]] = None,
) -> Optional[float]:
    """Get maximum allowed building area from rights data."""
    try:
...
def _calculate_legal_restrictions(
    asset,
    gis_data: Dict[str, Any],
    tabu_data: Dict[str, Any] = None,
    privilege_page_data: List[Dict[str, Any]] = None,
) -> Optional[str]:
    """
...
def _calculate_urban_renewal_potential(
    asset, gis_data: Dict[str, Any]
) -> Optional[str]:
    """
    ...
    try:
        ...
        if (
            tama38_key_areas
            and isinstance(tama38_key_areas, (list, tuple))
            and len(tama38_key_areas) > 0
        ):
        ...
        for permit in permits:
            if isinstance(permit, dict):
                ...
                if permit.get("permit_urban_renewal") or permit.get(
                    "sw_hitchadshut_ironit"
                ):
                ...
                if (
                    permit.get("sw_tama_38")
                    or permit.get("sw_tama_38_chadash")
                    or permit.get("sw_tama_38_tosefet")
                ):
                    urban_renewal_permits.append(permit)
                    continue
                ...
                if any(
                    keyword in permit_text
                    for keyword in ['תמ"א', "תמ״א", "חידוש", "פינוי", "בינוי", "tama"]
                ):
                    urban_renewal_permits.append(permit)
        ...
        plans = (
            gis_data.get("local_plans", [])
            or gis_data.get("city_plans", [])
            or gis_data.get("plans", [])
        )
        urban_renewal_plans = []
...
def _calculate_betterment_levy(
    asset, gis_data: Dict[str, Any], privilege_page_data: List[Dict[str, Any]] = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if asset.city:
            if any(city in asset.city for city in ["תל אביב", "רמת גן", "הרצליה"]):
                ...
                city in asset.city for city in ["פתח תקווה", "ראשון לציון", "חולון"]
            ):
...
def _calculate_height_analysis(
    asset, gis_data: Dict[str, Any]
    ...
    try:
        ...
        for permit in permits:
            if isinstance(permit, dict):
                ...
                if housing_units:
                    ...
                    estimated_floors = max(
                        1, housing_units // 2
                    )  # Assume 2 units per floor
        ...
        if privilege_data_list:
            privilege_data = (
                privilege_data_list[0]
                if isinstance(privilege_data_list, list)
                else privilege_data_list
            )
            if isinstance(privilege_data, dict):
                rights = privilege_data.get("rights", {})
...
def _calculate_setback_analysis(
    asset, gis_data: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if parcel_area and building_area:
            ...
            if coverage_ratio > 0.8:
                ...
                setback_analysis["violations"].append(
                    "ניצול בינוני - בדיקת נסיגות נדרשת"
                )
