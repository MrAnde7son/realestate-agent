...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!contentLength || contentLength === '0') {
      console.warn('Analytics session end: Empty request body');
    ...
    try {
      ...
      console.error('Analytics session end: Invalid JSON:', jsonError);
      return NextResponse.json(
    ...
    if (!body || Object.keys(body).length === 0) {
      console.warn('Analytics session end: Empty JSON body');
      return NextResponse.json(
