...
export default function AssetsPage() {
  ...
  const onSubmit = async (data: NewAsset) => {
    try {
      const body = {
        scope: {
          type: data.scopeType,
          value: data.scopeType === 'address' ? data.address :
                 data.scopeType === 'street' ? `${data.street} ${data.number}` :
                 data.scopeType === 'parcel' ? `${data.gush}/${data.helka}` :
                 data.address,
          city: data.city
        },
  ...
  const cityOptions = React.useMemo(
    () => Array.from(new Set(assets.map(l => l.city).filter(Boolean))) as string[],
    [assets]
  )
  const typeOptions = React.useMemo(
    () => Array.from(new Set(assets.map(l => l.type).filter(Boolean))) as string[],
    [assets]
  )
      ...
      <div className="p-6 space-y-6">
            ...
            {isAuthenticated ? (
              <Sheet open={open} onOpenChange={setOpen}>
                ...
                <SheetContent>
                  ...
                  {(form.watch('scopeType') === 'neighborhood' || form.watch('scopeType') === 'city') && (
                    <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            מציג {filteredAssets.length} מתוך {assets.length} נכסים עם נתוני שמאות מלאים
          </p>
        </div>
      </div>
    </DashboardLayout>
