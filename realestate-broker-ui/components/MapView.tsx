...
export default function MapView({
  ...
  // Use calc(100vh - 64px) to account for the header (h-16 = 64px)
  // On mobile, use fixed positioning to overlay everything
  const mapHeight = isMobile ? 'calc(100vh - 4rem)' : height
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground" style={isMobile ? { position: 'fixed' as const, top: '4rem', bottom: 0, left: 0, right: 0, zIndex: 40 } : { height: mapHeight }}>
  ...
  return (
    <div className={`relative w-full overflow-hidden ${isMobile ? 'rounded-none' : 'rounded-lg'}`} style={mapStyle}>
      {loading && (
      ...
      {onBackToTable && (
        <div className="absolute top-4 start-4 z-20">
          <Button
            ...
            className={`bg-white/90 backdrop-blur-sm ${isMobile ? 'h-8 w-8 p-0' : ''}`}
            title="חזרה לטבלה"
          >
            <ArrowLeft className={`h-4 w-4 ${isMobile ? '' : 'me-2 rtl:rotate-180'}`} />
      ...
      {/* Search Bar */}
      <div className={`absolute top-4 z-20 ${onBackToTable ? (isMobile ? 'start-12 end-4' : 'start-48 end-24') : 'start-4 end-24'}`}>
        ...
        <Popover open={showLayerControls} onOpenChange={setShowLayerControls}>
          <PopoverTrigger asChild>
            <Button 
              variant="outline" 
              size={isMobile ? "icon" : "sm"} 
              className={`bg-white/90 backdrop-blur-sm ${isMobile ? 'h-8 w-8 p-0' : 'h-8 px-2'}`}
            >
              <Layers className={`h-3.5 w-3.5 ${isMobile ? '' : 'me-1.5'}`} />
