...
def test_get_mcp_falls_back_without_dependency_support(monkeypatch, caplog):
    """FastMCP construction should fall back if dependency metadata isn't supported."""
    ...
    class DummyFastMCP:
        def __init__(self, name, instructions=None, **kwargs):
            if "dependencies" in kwargs:
                raise TypeError("unexpected keyword argument 'dependencies'")
            created_instances.append((name, instructions, kwargs))
            self._tool_manager = SimpleNamespace(_tools={})
    ...
    assert any("dependency metadata" in record.message for record in caplog.records)
    assert server._tools_registered is True
