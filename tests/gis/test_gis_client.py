...
def test_get_building_permits_no_download(monkeypatch, tmp_path):
    ...
    with mock.patch("requests.post", side_effect=fake_post):
        results = gs.get_building_permits(
            178000, 665000, radius=30, download_pdfs=False
        )
...
def test_get_building_permits_with_download(monkeypatch, tmp_path):
    ...
    with (
        ...
        results = gs.get_building_permits(
            178000, 665000, radius=30, download_pdfs=True, save_dir=str(save_dir)
        )
        assert len(results) == 1
...
is_house      = '';
    ...
    def fake_get(
        ...
        if "medamukdam/fr_asp/fr_meda_main.asp?gush=1234&helka=56" in url:
            r = requests.Response()
            r.status_code = 200
    ...
    with (
        ...
        assert isinstance(result["parcels"], list), (
            "Parcels should be a list for HTML content"
        )
...
def test_get_building_privilege_page_no_blocks(monkeypatch, tmp_path):
    ...
    def fake_post(url, data=None, headers=None, timeout=30, **kwargs):
        if "MapServer/525/query" in url:  # blocks layer
            ...
            "MapServer/524/query" in url
        ):  # parcels layer - won't be called due to early return
...
def test_get_building_privilege_page_download_failure(monkeypatch, tmp_path):
    ...
    def fake_get(
        ...
        if "medamukdam/fr_asp/fr_meda_main.asp" in url:
...
def test_get_building_privilege_page_custom_save_dir(monkeypatch, tmp_path):
    ...
    with (
        ...
        result = gs.get_building_privilege_page(
            178000, 665000, save_dir=str(custom_dir)
        )
