...
def test_parse_zchuyot_pdf():
    ...
    assert "national_regional" in in_force, (
        "In-force plans should contain national_regional section"
    )

    in_planning = plans["in_planning"]
    assert "citywide" in in_planning, "In-planning should contain citywide section"
    assert "national_regional" in in_planning, (
        "In-planning should contain national_regional section"
    )
    ...
    for section_name, section_data in in_force.items():
        assert isinstance(section_data, list), (
            f"Plan section {section_name} should be a list"
        )

    for section_name, section_data in in_planning.items():
        assert isinstance(section_data, list), (
            f"In-planning section {section_name} should be a list"
        )
    ...
    assert 0 < len(result["raw_preview"]) <= 20000, (
        "Raw preview should have a sensible length"
    )

    print(f"Successfully parsed PDF with {len(result.get('policies', []))} policies")
    print(f"Basic info: {basic}")
    print(
        f"Total in-force plans: {sum(len(plans['in_force'][k]) for k in plans['in_force'])}"
    )
    print(
        f"Total in-planning plans: {sum(len(plans['in_planning'][k]) for k in plans['in_planning'])}"
    )
            ...
            if plan_list:  # If plans exist in this section
                for plan in plan_list:
                    assert isinstance(plan, dict), (
                        f"Plan should be a dictionary in {plan_type}.{section_name}"
                    )
                    # Each plan should have at least a plan_number or name
                    has_plan_info = any([plan.get("plan_number"), plan.get("name")])
                    assert has_plan_info, (
                        f"Plan should have at least plan_number or name in {plan_type}.{section_name}"
                    )
    ...
    if all_links:
        for url in all_links:
            ...
            assert not url.endswith((".", ",", ";", "]", ">")), (
                f"URL should not end with punctuation: {url}"
            )

    print(
        f"Content validation passed. Extracted {len(all_links)} URLs and basic info: {basic}"
    )
...
def test_parse_zchuyot_pdf_plan_structure():
    ...
    for section_name, plan_list in in_force.items():
        ...
        for plan in plan_list:
            assert isinstance(plan, dict), (
                f"Plan in {section_name} should be a dictionary"
            )
            ...
            for date_field in ("deposit_date", "effective_date"):
                if plan.get(date_field) is not None:
                    assert isinstance(plan[date_field], str), (
                        f"{date_field} should be a string when present"
                    )

            # Check URLs field if present
            if "urls" in plan:
                assert isinstance(plan["urls"], list), (
                    f"Plan URLs should be a list in {section_name}"
                )
                for url in plan["urls"]:
                    assert isinstance(url, str), (
                        f"Plan URL should be a string in {section_name}"
                    )
                    assert url.startswith(("http://", "https://")), (
                        f"Plan URL should be valid: {url}"
                    )
    ...
    for section_name, plan_list in in_planning.items():
        assert isinstance(plan_list, list), (
            f"In-planning {section_name} should be a list"
        )
        ...
        for plan in plan_list:
            ...
            if "plan_number" in plan:
                assert plan["plan_number"] not in (None, ""), (
                    f"Plan number should not be empty in {section_name}"
                )
            if "name" in plan:
                assert plan["name"] is not None, (
                    f"Plan name should not be None in {section_name}"
                )
...
def test_parse_zchuyot_pdf_rights_and_policy():
    ...
    assert text_value is None or isinstance(text_value, str), (
        "Rights.text should be a string or None"
    )
    ...
    for field in list_fields:
        if field in rights:
            assert isinstance(rights[field], list), (
                f"Rights field {field} should be a list"
            )

    if "building_lines" in rights:
        assert all(isinstance(item, str) for item in rights["building_lines"]), (
            "Building lines should be strings"
        )
    ...
    if "floor_percentages" in rights:
        for value in rights["floor_percentages"]:
            assert isinstance(value, (int, float)), (
                "Floor percentage entries should be numeric"
            )
    ...
    if "floor_percentages_detailed" in rights:
        for item in rights["floor_percentages_detailed"]:
            ...
            assert "percentage" in item and isinstance(
                item["percentage"], (int, float)
            ), "Detailed percentage must be numeric"
    ...
    for field in numeric_fields:
        if field in rights and rights[field] is not None:
            assert isinstance(rights[field], (int, float)), (
                f"Rights field {field} should be numeric when present"
            )
    ...
    if policies:
        for pol in policies:
            ...
            assert any(pol.values()), (
                "Policy item should have at least one non-empty value"
            )

    print(
        f"Rights and policy validation passed. Found {len(rights)} rights keys and {len(policies)} policy items"
    )
