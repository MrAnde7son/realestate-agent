        ...
        def mock_on_response(response):
            ...
            if hasattr(response, "url") and hasattr(response, "status"):
                ...
                response.json = lambda: {
                    "data": {"items": [{"planId": "12345", "planName": "Test Plan"}]}
                }
                response.text = (
                    lambda: '{"data": {"items": [{"planId": "12345", "planName": "Test Plan"}]}}'
                )
...
class TestMavatScraperIntegration:
    ...
    @pytest.mark.skip(
        reason="Integration test - requires network access and may be slow"
    )
