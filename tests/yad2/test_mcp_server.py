...
class TestFetchListings:
    ...
    async def test_fetch_listings_success(
        self, mock_api_client, mock_ctx, mock_listing
    ):
        ...
        result = await fetch_listings_func(
            mock_ctx, city="5000", property="1", listing_type="sale"
        )
    ...
    async def test_fetch_listings_no_results(self, mock_api_client, mock_ctx):
        ...
        # Call function
        result = await fetch_listings_func(mock_ctx, city="5000")
    ...
    @pytest.mark.asyncio
    @patch("yad2.mcp_server._api_client")
    async def test_fetch_listings_with_hebrew_property(
        self, mock_api_client, mock_ctx, mock_listing
    ):
        ...
        result = await fetch_listings_func(mock_ctx, property="דירה", city="5000")
...
class TestFetchContactInfo:
    ...
    @pytest.mark.asyncio
    @patch("yad2.mcp_server._api_client")
    async def test_fetch_contact_info_success(
        self, mock_api_client, mock_ctx, mock_contact_info
    ):
...
class TestFetchProjectAutocomplete:
    ...
    async def test_fetch_project_autocomplete_success(self, mock_api_client, mock_ctx):
        ...
        # Verify API was called correctly
        mock_api_client.fetch_project_autocomplete.assert_called_once_with(
            "Test Project"
        )

    @pytest.mark.asyncio
    @patch("yad2.mcp_server._api_client")
    async def test_fetch_project_autocomplete_not_found(
        self, mock_api_client, mock_ctx
    ):
...
class TestFetchLocationAutocomplete:
    ...
    async def test_fetch_location_autocomplete_success(self, mock_api_client, mock_ctx):
        ...
        mock_params.get_active_parameters = Mock(
            return_value={"city": 5000, "neighborhood": 203}
        )
        mock_api_client.fetch_location_autocomplete.return_value = mock_params

        # Call function
        result = await fetch_location_autocomplete_func(
            mock_ctx, search_text="רמת החייל"
        )
    ...
    @pytest.mark.asyncio
    @patch("yad2.mcp_server._api_client")
    async def test_fetch_location_autocomplete_not_found(
        self, mock_api_client, mock_ctx
    ):
        ...
        # Call function
        result = await fetch_location_autocomplete_func(
            mock_ctx, search_text="Nonexistent Location"
        )
...
class TestFetchLatestDeals:
    ...
    @pytest.mark.asyncio
    @patch("yad2.mcp_server._api_client")
    async def test_fetch_latest_deals_success(
        self, mock_api_client, mock_ctx, mock_deal
    ):
        ...
        # Call function
        result = await fetch_latest_deals_func(mock_ctx, city=5000, limit=10)
