...
class InteractiveCLI:
    ...
    def _handle_parameter_input(self, param_name):
        ...
        if param_name not in self.reference.PARAMETER_INFO:
            print(
                "❓ Unknown parameter '{}'. Type 'help' for available parameters.".format(
                    param_name
                )
            )
            return
        ...
        value = input(
            "Enter value for {} (or press Enter to skip): ".format(param_name)
        ).strip()
    ...
    def _offer_scraping(self, params):
        ...
        if choice == "1":
            ...
            try:
                max_pages = input(
                    "Number of pages to scrape (1-10, default 2): "
                ).strip()
